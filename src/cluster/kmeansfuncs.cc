// ==========================================================================
// Header file for some kmeansfuncs namespace
// ==========================================================================
...
/*
  ...
  This program is a re-implementation of the k-means clustering algorithm.  It
...
/* Compiling
   ...
   This program has been successfully compiled by me on a Linux platform with gcc,
   but you should be able to compile it on any unix platform supporting pthread
   with a standard C99 compiler with minimal modification.  I usually use the
   following command on my platform.

   gcc -Wall -O3 -g -std=c99 -o kmeans kmeans.c -lpthread -lm

   For simplicity and speed, some parameters are hard coded into this file,
   please go to the compile time parameter section to adjust these parameters.
   You can override those parameters by defining them on the compiling commandline.
   These parameters allow you to determine which optimization to use (BLAS or
   kd-tree), whether or not to do value checking and what text file field and line
   delimitors to use.  Note that the following two features require extra software
   support:
   1. USE_MPI : you need an MPI platform.  I use mpich, but openmpi or
   others should also do.
   2. USE_BLAS: you need a BLAS library.  I use ATLAS, but Intel MKL should
   be a better choice on Intel platforms.

   Note that USE_BLAS and USE_KD_TREE cannot be enabled together and by default
   none is enabled.  Following is a guideline to choose between BLAS and kd-tree
   or neither:
...
/* Input/Output File Format:
   ...
   This program is quite flexible on input file format.  It supports both binary
   (preferred) and text format.
...
/* Usage:
   ...
   If initial clusters are not given via the -S parameter,the initial cluster centers will be sampled at random from the first p blocks read.  If p is not provided or if the p provided is too small, the least number of blocks that holds more than K vectors will be used.
   -z z output cluster sizes to file z


   The non-MPI program catches SIGTERM and SIGINT when clustering is going on.  When SIGTERM is received (killall kmeans), it will abort the current loop and writes the results of the previous loop; when SIGINT (type Ctrl-C in commandline), it will report the current status and continue working.  To kill the program immmediately, use killall -9 kmeans.

   Some extra note:
   *  The program creates on reading thread for each input file.  Splitting the input data
   will only improve throughput when they are stored on independent disk drives.  Multiple
   files on a single disk drive can only slow down reading.
   *  When using with MPI, if -l is specified, then the input files are treated as different
   files with the same names on different nodes.  If -l is not given, then the input files
   are treated as the same files on different nodes; the files are logically splitted and
   each node reads a different portion from each file.
   *  The parameter -P allows you to write data belonging to different cluster center to
   separate files.  It is intended to be used to partition the dataset according to
   existing clustering result with the following commandline:
            
   kmeans -S <previous-clustering-result> -P output input...

   If the program is interrupted, the integrity of
   the files are not guaranteed (while the program always tries to write the clustering result
   of the last finished iteration before quitting).  Since only one thread is used for
   writing the partitioned data, partitioning can be the bottleneck when dataset is large.
*/
