// ==========================================================================
// Program GENERATE_ERS is a variant of EXTREMAL numbers which imports
// a small image chip containing a manually selected individual digit.
// It runs this number chip through our extremal region battery.  The
// set of output connected components is then sorted according to
// their pixel size.  The median extremal region is taken to be the
// best representative for the input individual digit.
// ==========================================================================
// Last updated on 4/19/14; 4/21/14; 4/27/14
// ==========================================================================
...
connected_component_t* export_median_image(
   bool descending_threshold_flag,LEVELS_CCS_MAP& levels_ccs_map,
   texture_rectangle* texture_rectangle_ptr);
void export_connected_components_for_particular_level(
   ...
   texture_rectangle* texture_rectangle_ptr);
void export_median_connected_component(
...
// --------------------------------------------------------------------------
// Method export_median_image() loops over all levels within input
// STL map levels_ccs_map.  For each it extracts the threshold and
// associated connected components.  Each level's connected components
// are visualized within a colored image.
...
//              << " cc_ptrs.size = " << cc_ptrs.size() << endl;
         for (int cc=0; cc<cc_ptrs.size(); cc++)
         {
...
//              << " cc_ptrs.size = " << cc_ptrs.size() << endl;
   ...
   cout << "n_candidate_ccs = " << n_candidate_ccs << endl;
   vector<int> cc_index;
   vector<int> cc_size;
   ...
   templatefunc::Quicksort(cc_size, cc_index);

   int median_index = cc_index[n_candidate_ccs/2];
   connected_component_t* median_cc_ptr = candidate_cc_ptrs[median_index];
   export_median_connected_component(
      descending_threshold_flag,median_cc_ptr,
      texture_rectangle_ptr);
...
// --------------------------------------------------------------------------
// Method export_median_connected_component()

void export_median_connected_component(
   bool descending_threshold_flag, connected_component_t* cc_ptr,
   texture_rectangle* texture_rectangle_ptr)
{
//   cout << "inside export_median_connected_component()" << endl;
...
//      image_filename,descending_threshold_flag,levels_ccs_map);
   ...
   connected_component_t* median_cc_ptr = export_median_image(
      descending_threshold_flag,levels_ccs_map,texture_rectangle_ptr);
