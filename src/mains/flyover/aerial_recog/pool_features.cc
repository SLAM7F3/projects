// ==========================================================================
// POOL_FEATURES reads in a set of positive or negative class images
...
// need not equal 32.  It also imports a precomputed character
// dictionary along with an inverse square root covariance matrix.
// Looping over each input window file, POOL_FEATURES extracts 8x8
// RGB or grey-scale intensity patches in each window for every
// possible ~25x~25 pixel offset.  Each 8x8 patch is projected onto
// the dictionary and converted into a K-dimensional histogram. The
// histogram descriptors are averaged together within 3x3 regions of
// the ~32x~32 window.  So the final descriptor for each ~32x~32
// window is a 9*K dimensional vector.  9*K dimensional window
// descriptors for all valid ~32x~32 windows not basically equal to
// the zero vector are exported to windows_descriptors.hdf5.

// 			      pool_features

// ==========================================================================
// Last updated on 8/23/12; 5/31/14; 6/1/14; 6/13/14; 6/22/14
// ==========================================================================
...
// ==========================================================================
{
   ...
   string pool_char="p";
   cout << "Enter 'p' to pool positive features or 'n' to pool negative features:"
        << endl;
   cin >> pool_char;

   bool positives_input_flag=true;
   if (pool_char=="n")
   {
      positives_input_flag=false;
   }

   string training_images_subdir;
   if (positives_input_flag)
   {
      training_images_subdir=trees_subdir+"all_32x32_tree_chips/";
   }
   else
   {
      training_images_subdir=trees_subdir+"all_32x32_negative_chips/";
   }
...
//   int i_stop=1000;
   ...
   if (positives_input_flag)
   {
      banner="Pooling positive features for "+stringfunc::number_to_string(
         i_stop-i_start)+" images";
   }
   else
   {
      banner="Pooling negative features for "+stringfunc::number_to_string(
         i_stop-i_start)+" images";
   }
...
//            outputfunc::enter_continue_char();
            ...
            else if (!positives_input_flag && n_zero_descriptors > 100)
            {
               continue;
            }
         }
   ...
   string image_type_label="_positive";
   if (!positives_input_flag)
   {
      image_type_label="_negative";
   }
