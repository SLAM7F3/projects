...
// ==========================================================================
// Last modified on 10/14/08; 10/24/08
// ==========================================================================
...
#include "video/imageprojfuncs.h"
#include "video/readparamsfuncs.h"

using std::cout;
using std::endl;

const double CLOCKS_PER_MSEC=(CLOCKS_PER_SEC/1000.0);
const int MAXITER=100;

// pointers to additional data, used for computed image projections
// and their jacobians

struct globs_{
      double *intrcalib; /* the 5 intrinsic calibration parameters in
                          * the order [fu, u0, v0, ar, skew], where ar
                          * is the aspect ratio fv/fu.  Used only when
                          * calibration is fixed for all cameras;
                          * otherwise, it is null and the intrinsic
                          * parameters are included in the set of
                          * motion parameters for each camera
                          */
      int nccalib; /* number of calibration parameters that must be
                    * kept constant.  0: all paremeters are free 1:
                    * skew is fixed to its initial value, all other
                    * parameters vary (i.e. fu, u0, v0, ar) 2: skew
                    * and aspect ratio are fixed to their initial
                    * values, all other parameters vary (i.e. fu, u0,
                    * v0) 3: meaningless 4: skew, aspect ratio and
                    * principal point are fixed to their initial
                    * values, only the focal length varies (i.e. fu)
                    * >=5: meaningless Used only when calibration
                    * varies among cameras
                    */

      int cnp, pnp, mnp; /* dimensions */

      double *ptparams; /* needed only when bundle adjusting for
                         * camera parameters only */
      double *camparams; /* needed only when bundle adjusting for
...
static void img_projRTS(
   int j, int i, double *aj, double *bi, double *xij, void *adata)
{
   double* Kparms;
   struct globs_* gl;
   ...
   imageprojfunc::calcImgProj(Kparms, aj, aj+3, bi, xij); 
   // 3 is the quaternion's vector part length
}
...
static void img_projRTS_jac(int j, int i, double *aj, double *bi, 
                            double *Aij, double *Bij, void *adata)
{
   ...
   imageprojfunc::calcImgProjJacRTS(
      Kparms, aj, aj+3, bi, (double (*)[6])Aij, (double (*)[3])Bij); // 3 is the quaternion's vector part length
}
...
static void img_projRT(int j, int i, double *aj, double *xij, void *adata)
{
   ...
   imageprojfunc::calcImgProj(
      Kparms, aj, aj+3, ptparams+i*pnp, xij); // 3 is the quaternion's vector part length
}
...
static void img_projRT_jac(int j, int i, double *aj, double *Aij, void *adata)
{
   ...
   imageprojfunc::calcImgProjJacRT(
      Kparms, aj, aj+3, ptparams+i*pnp, (double (*)[6])Aij); // 3 is the quaternion's vector part length
}
...
static void img_projS(int j, int i, double *bi, double *xij, void *adata)
{
   ...
   imageprojfunc::calcImgProj(Kparms, aj, aj+3, bi, xij); // 3 is the quaternion's vector part length
}
...
static void img_projS_jac(int j, int i, double *bi, double *Bij, void *adata)
{
   ...
   imageprojfunc::calcImgProjJacS(
      Kparms, aj, aj+3, bi, (double (*)[3])Bij); // 3 is the quaternion's vector part length
}
...
static void img_projsRTS_x(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx, void *adata)
{
   ...
   for(j=0; j<m; ++j){
      ...
      for(i=0; i<nnz; ++i){
         ...
         imageprojfunc::calcImgProj(Kparms, pqr, pt, ppt, pmeas); // evaluate Q in pmeas
      }
   }
}
...
static void img_projsRTS_jac_x(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *jac, void *adata)
{
   ...
   for(j=0; j<m; ++j){
      ...
      for(i=0; i<nnz; ++i){
         ...
         imageprojfunc::calcImgProjJacRTS(Kparms, pqr, pt, ppt, (double (*)[6])pA, (double (*)[3])pB); // evaluate dQ/da, dQ/db in pA, pB
      }
   }
}
 ...
 *
 */
static void img_projsRT_x(
   double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx, void *adata)
{
   register int i, j;
   int cnp, pnp, mnp;
...
static void img_projsRT_jac_x(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *jac, void *adata)
{
   ...
   for(j=0; j<m; ++j){
      ...
      for(i=0; i<nnz; ++i){
         ...
         imageprojfunc::calcImgProjJacRT(Kparms, pqr, pt, ppt, (double (*)[6])pA); // evaluate dQ/da in pA
      }
   }
}
...
static void img_projsS_jac_x(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *jac, void *adata)
{
   ...
   for(j=0; j<m; ++j){
      ...
      for(i=0; i<nnz; ++i){
         ...
         imageprojfunc::calcImgProjJacS(Kparms, pqr, pt, ppt, (double (*)[3])pB); // evaluate dQ/da in pB
      }
   }
}
...
{
   imageprojfunc::calcImgProj(aj, aj+5, aj+5+3, bi, xij); // 5 for the calibration + 3 for the quaternion's vector part
}
...
static void img_projKRTS_jac(int j, int i, double *aj, double *bi, double *Aij, double *Bij, void *adata)
{
   ...
   imageprojfunc::calcImgProjJacKRTS(aj, aj+5, aj+5+3, bi, (double (*)[5+6])Aij, (double (*)[3])Bij); // 5 for the calibration + 3 for the quaternion's vector part
...
static void img_projKRT(int j, int i, double *aj, double *xij, void *adata)
{
   ...
   imageprojfunc::calcImgProj(aj, aj+5, aj+5+3, ptparams+i*pnp, xij); // 5 for the calibration + 3 for the quaternion's vector part
}
...
static void img_projKRT_jac(int j, int i, double *aj, double *Aij, void *adata)
{
   ...
   imageprojfunc::calcImgProjJacKRT(aj, aj+5, aj+5+3, ptparams+i*pnp, (double (*)[5+6])Aij); // 5 for the calibration + 3 for the quaternion's vector part
...
static void img_projKS_jac(int j, int i, double *bi, double *Bij, void *adata)
{
   ...
   imageprojfunc::calcImgProjJacS(aj, aj+5, aj+5+3, bi, (double (*)[3])Bij); // 5 for the calibration + 3 for the quaternion's vector part
}
...
static void img_projsKRTS_x(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx, void *adata)
{
   ...
   for(j=0; j<m; ++j){
      ...
      for(i=0; i<nnz; ++i){
         ...
         imageprojfunc::calcImgProj(pcalib, pqr, pt, ppt, pmeas); // evaluate Q in pmeas
      }
   }
}
...
static void img_projsKRTS_jac_x(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *jac, void *adata)
{
   ...
   for(j=0; j<m; ++j){
      ...
      for(i=0; i<nnz; ++i){
         ...
         imageprojfunc::calcImgProjJacKRTS(pcalib, pqr, pt, ppt, (double (*)[5+6])pA, (double (*)[3])pB); // evaluate dQ/da, dQ/db in pA, pB
...
static void img_projsKRT_jac_x(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *jac, void *adata)
{
   ...
   for(j=0; j<m; ++j){
      ...
      for(i=0; i<nnz; ++i){
         ...
         imageprojfunc::calcImgProjJacKRT(pcalib, pqr, pt, ppt, (double (*)[5+6])pA); // evaluate dQ/da in pA
...
static void img_projsKS_jac_x(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *jac, void *adata)
{
   ...
   for(j=0; j<m; ++j){
      ...
      for(i=0; i<nnz; ++i){
         ...
         imageprojfunc::calcImgProjJacS(pcalib, pqr, pt, ppt, (double (*)[3])pB); // evaluate dQ/da in pB
      }
   }
}
...
void sba_driver(
   char *camsfname, char *ptsfname, char *calibfname, 
   ...
   int filecnp)
{
   ...
   static const double SBA_MAX_REPROJ_ERROR =4.0; // max motion only
                                                  // reprojection
                                                  // error

   const int BA_NONE=-1;
   const int BA_MOTSTRUCT=0;
   const int BA_MOT=1;
   const int BA_STRUCT=2;               
   const int BA_MOT_MOTSTRUCT=3;

   /* NOTE: readInitialSBAEstimate() sets covimgpts to NULL if no covariances are supplied */

   readparamsfunc::readInitialSBAEstimate(
      camsfname, ptsfname, cnp, pnp, mnp, caminfilter, filecnp, //NULL, 0, 
      &nframes, &numpts3D, &numprojs, &motstruct, &imgpts, &covimgpts, 
      &vmask);
   ...
   if(calibfname){ // read intrinsics only if fixed for all cameras
      readparamsfunc::readCalibParams(calibfname, K);
      ical[0]=K[0]; // fu
      ical[1]=K[2]; // u0
      ical[2]=K[5]; // v0
      ical[3]=K[4]/K[0]; // ar
...
//   prnt=BA_NONE;
   ...
   switch(howto){
      case BA_MOTSTRUCT: /* BA for motion & structure */
         ...
         if(expert)
            ...
            n=sba_motstr_levmar_x(
               numpts3D, nframes, nconstframes, vmask, motstruct, 
               cnp, pnp, imgpts, covimgpts, mnp,
               fixedcal? img_projsRTS_x : img_projsKRTS_x, 
               analyticjac? (fixedcal? img_projsRTS_jac_x : img_projsKRTS_jac_x) : NULL,
   ...
   switch(prnt){
      ...
      case BA_MOTSTRUCT:
         cout << "------------------------------------------------------"
              << endl;
         cout << "Revised Motion Data:" << endl;
         readparamsfunc::printSBAMotionData(
            motstruct, nframes, cnp, camoutfilter, filecnp);
         cout << "------------------------------------------------------"
              << endl;
         cout << "Revised Structure Data:" << endl;
         readparamsfunc::printSBAStructureData(
            motstruct, nframes, numpts3D, cnp, pnp);
         break;
 ...
 * positive scalar part. Remaining parameters are left unchanged.
 *

 * Input parameter layout: intrinsics (5, optional), rot. quaternion
 * (4), translation (3)

 * Output parameter layout: intrinsics (5, optional), rot. quaternion
 * vector part (3), translation (3)
...
void quat2vec(double *inp, int nin, double *outp, int nout)
{
    ...
    * positive; if not, negate the quaternion since two quaternions q
 ...
 *
 * Input parameter layout: intrinsics (5, optional), rot. quaternion
 * vector part (3), translation (3) Output parameter layout:
 * intrinsics (5, optional), rot. quaternion (4), translation (3)
 */
...
{
   int cnp=6, // # params defining single camera 3 rot + 3 trans params
      pnp=3, // # params defining single 3D point (euclidean 3D points)
      mnp=2; // # params defining image point (U,V)
   ...
   if(argc==4){
      ...
      sba_driver(argv[1], argv[2], argv[3], cnp, pnp, mnp, 
                 quat2vec, vec2quat, cnp+1);
   }
   else{
      fprintf(stderr, "Starting BA with varying intrinsic parameters\n");
      cnp+=5; /* 5 more params for calibration */
      sba_driver(argv[1], argv[2], NULL, cnp, pnp, mnp, 
                 quat2vec, vec2quat, cnp+1);
   }
}
