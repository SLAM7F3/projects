// ========================================================================
// Program QTLOSTSERVER is a variant of program QTLOS.  It runs a
// "black-box" server for LOST which does not display any 3D
// OpenSceneGraph window.  We spun off this LOST variant in January
// 2012 for Pyxis program purposes.
// ========================================================================
// Last updated on 12/22/10; 12/24/10; 1/14/12
// ========================================================================
...
// As of 1/25/12, we assume LOST black box server only works with
...
//    window_mgr_ptr->realize();
   ...
   cout << "in main before set AC modelsgroup_ptr = "
        << Aircraft_MODELSGROUP_ptr << endl;
...
//      Earth_ptr->get_drawable_group_ptr()->setNodeMask(0);
   ...
   cout << "Before infinite event loop in main()" << endl;
   while (true)
   {
      window_mgr_ptr->partial_process();
      app.processEvents();
   }
