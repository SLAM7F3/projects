// ========================================================================
// Program QTTOC11 is a variant of QTLOS constructed for red-actor
// path network discovery.  It is specialized for displaying &
// exploiting FOB Blessing ALIRT imagery.  Double click on QTTOC11
// icon in order to start this program.  Choose "Afghanistan/Pakistan"
// for world region.  FOB Blessing ALIRT data will subsequently show
// up in OpenSceneGraph window.  (Need to have 1 TB Disk #6 plugged
// into LOST laptop, for it contains FOB Blessing ALIRT imagery.)

// [   Press "A" twice to enter into Manipulate Annotation Mode.  
//     Then press "r" to read in Start and Stop signpost locations.     ]

// See README.pathfinding for keystroke instructions on running
// QTTOC11

// ========================================================================
// Last updated on 5/19/11; 5/20/11; 7/2/11
// ========================================================================
...
//   double world_time_step_in_secs_per_frame=0.5;	// secs
   double world_time_step_in_secs_per_frame=30;		// secs
...
// For TOC11, we work with ALIRT imagery over FOB Blessing rather than
// DTED-2 tiles for all of Afghanistan and Pakistan:
   
   string map_countries_name=LOS_server.get_map_countries_name();
   if (ladar_height_data_flag && map_countries_name=="Afghanistan")
   {
      map_countries_name="FOB_Blessing";
//      cloudpassinfo_ptr->set_longitude_lo(70.65);
//      cloudpassinfo_ptr->set_longitude_hi(72);
//      cloudpassinfo_ptr->set_latitude_lo(34.86);
//      cloudpassinfo_ptr->set_latitude_hi(36);

      cloudpassinfo_ptr->set_longitude_lo(60);
      cloudpassinfo_ptr->set_longitude_hi(78);
      cloudpassinfo_ptr->set_latitude_lo(24);
...
// latitude-longitude grids and associated earth regions:
   ...
   clouds_group.set_Operations_ptr(&operations);
...
//   double scalar_value_stop=basic_math::round(z_max * ft_per_km);
   double scalar_value_start=1000;	// meters	FOB Blessing
   double scalar_value_stop=4000;	// meters	FOB Blessing
   string title="Terrain altitude (meters)";
...
// Instantiate a MyDatabasePager to handle paging of files from disk:
   ...
   HiresDataVisitor* HiresDataVisitor_ptr=clouds_group.
      get_HiresDataVisitor_ptr();
   HiresDataVisitor_ptr->setDatabasePager(MyDatabasePager_ptr);
...
// Instantiate PolyLines and MODEL decorations groups:
   ...
   Flight_PolyLinePickHandler_ptr->set_Allow_Insertion_flag(false);

   PolyLinesKeyHandler* Flight_PolyLinesKeyHandler_ptr=
      decorations.get_PolyLinesKeyHandler_ptr(0);
   Flight_PolyLinesKeyHandler_ptr->set_Allow_Insertion_flag(false);
   ...
   Aircraft_MODELSGROUP_ptr->set_loiter_at_track_end_flag(false);
   ...
   Aircraft_MODELSGROUP_ptr->set_max_ground_Z(PointCloud_ptr->
   	get_max_value(2));
...
//   Aircraft_MODELSGROUP_ptr->set_aircraft_altitude(9000);
   ...
   double frac_offset;
   cout << "Enter flightpath fraction offset:" << endl;
   cin >> frac_offset;
   Aircraft_MODELSGROUP_ptr->set_flightpath_fraction_offset(frac_offset);

// Instantiate PolyLinesGroup to hold ground paths.  Interact
// with these Polylines in INSERT[MANIPULATE]_POLYLINE mode rather
// than INSERT[MANIPULATE]_LINE mode:

   AnimationController* groundpath_AnimationController_ptr=
      new AnimationController;
   root->addChild(groundpath_AnimationController_ptr->get_OSGgroup_ptr());

   AnimationController_ptr->set_master_AnimationController_ptr(
      groundpath_AnimationController_ptr);
   cout << "AnimationController_ptr = " << AnimationController_ptr << endl;
   cout << "groundpath_AnimationController_ptr = "
        << groundpath_AnimationController_ptr << endl;

   clouds_group.set_AnimationController_ptr(
      groundpath_AnimationController_ptr);
   Aircraft_MODELSGROUP_ptr->set_AnimationController_ptr(
      groundpath_AnimationController_ptr);
   operations.get_ImageNumberHUD_ptr()->set_AnimationController_ptr(
      groundpath_AnimationController_ptr);

   PolyLinesGroup* Groundpath_PolyLinesGroup_ptr=decorations.add_PolyLines(
      ndims,passes_group.get_pass_ptr(cloudpass_ID));
   cout << "Groundpath_PolyLinesGroup_ptr = "
        << Groundpath_PolyLinesGroup_ptr << endl;
   Groundpath_PolyLinesGroup_ptr->set_width(4);
   Groundpath_PolyLinesGroup_ptr->set_AnimationController_ptr(
      groundpath_AnimationController_ptr);

   PolyLinePickHandler* Groundpath_PolyLinePickHandler_ptr=
      decorations.get_PolyLinePickHandler_ptr(1);
   Groundpath_PolyLinePickHandler_ptr->set_regular_vertex_spacing(10); 
	// meters
   Groundpath_PolyLinePickHandler_ptr->set_form_polyhedron_skeleton_flag(
      false);
   Groundpath_PolyLinePickHandler_ptr->set_Allow_Insertion_flag(true);
   clouds_group.set_PolyLinesGroup_ptr(Groundpath_PolyLinesGroup_ptr);

   PolyLinesKeyHandler* Groundpath_PolyLineKeyHandler_ptr=
      decorations.get_PolyLinesKeyHandler_ptr(1);
   cout << "Groundpath_PolyLineKeyHandler_ptr = "
        << Groundpath_PolyLineKeyHandler_ptr << endl;
   Groundpath_PolyLineKeyHandler_ptr->set_Allow_Insertion_flag(true);
...
// Instantiate SignPostsGroup to hold ground target locations:
   ...
   GroundTarget_SignPostsGroup_ptr->set_AnimationController_ptr(
      groundpath_AnimationController_ptr);
   Aircraft_MODELSGROUP_ptr->set_GroundTarget_SignPostsGroup_ptr(
      GroundTarget_SignPostsGroup_ptr);

// Instantiate tracks_group to hold ground paths:

   tracks_group* groundtracks_group_ptr=new tracks_group();
   clouds_group.set_tracks_group_ptr(groundtracks_group_ptr);
   GroundTarget_SignPostsGroup_ptr->set_tracks_group_ptr(
      groundtracks_group_ptr);
...
// FOB Blessing bbox params:

   TilesGroup_ptr->set_ladar_data_bbox(
      650300,685300,3857699,3887699,5000,5000);

   clouds_group.set_TilesGroup_ptr(TilesGroup_ptr);
...
// FAKE FAKE:  Thurs Aug 11 at 2:42 pm
// Comment out next line for TOC11 viewgraph purposes only...
...
// calls the CustomManipulator's home() method:
   ...
   LOS_server.set_AnimationController_ptr(groundpath_AnimationController_ptr);
   LOS_server.set_reset_AnimationController_start_stop_times_flag(false);
...
//      Earth_ptr->get_drawable_group_ptr()->setNodeMask(0);
   ...
   while( !window_mgr_ptr->done() )
   {
      ...
      app.processEvents();
      AnimationController_ptr->sync_to_master();
   }
