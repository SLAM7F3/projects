// As of 6/6/04, we'd like to split this program up into
// SUBTRACT_GROUND & EXTRACT_FEATURES.  

// ==========================================================================
// Program GROUND_BLDGS reads in and interprets raw ALIRT xyzp data.
// In particular, it attempts to classify trees, buildings, grass and
// roadsides on a per-pixel basis
// ==========================================================================
// Last updated on 7/5/05; 4/24/06; 10/30/07
// ==========================================================================
...
// ==========================================================================
{
   ...
   string logfilename=sysfunc::get_projectsrootdir()+
      "src/mains/alirt_acc/ground.logfile";
...
// Chimney footprint dimensions:
   ...
   cityimage.use_bbox_to_resize_ladarimage(delta_x,delta_y);
   ladarfunc::null_data_outside_bbox(
      cityimage.get_data_bbox_ptr(),cityimage.get_z2Darray_ptr());
//   cityimage.writeimage("raw_zimage",cityimage.get_z2Darray_ptr(),true);   
//   cityimage.writeimage("raw_pimage",cityimage.get_p2Darray_ptr(),false,
//                       ladarimage::p_data);   
...
// height imagery:
   ...
   ladarfunc::median_fill_image(
      3,cityimage.get_data_bbox_ptr(),cityimage.get_z2Darray_ptr());
   ladarfunc::remove_isolated_outliers(
      cityimage.get_data_bbox_ptr(),cityimage.get_z2Darray_ptr());
   string filled_zimage_filenamestr=cityimage.get_imagedir()+
      "filled_zimage.xyzp";
   xyzpfunc::write_xyzp_data(
      cityimage.get_z2Darray_ptr(),cityimage.get_p2Darray_ptr(),
      filled_zimage_filenamestr);
//   cityimage.writeimage("filled_zimage",cityimage.get_z2Darray_ptr());   
...
//   const double spatial_resolution=0.3;	// meter
   ...
   cityimage.estimate_ground_surface_using_gradient_info(
      ...
      zground_silhouetted_twoDarray_ptr);
...
//   cityimage.dynamic_colortable=true;
//   cityimage.dynamic_colortable_minz=-0.01;
//   cityimage.dynamic_colortable_maxz=1.01;
//   cityimage.writeimage("zlo",zhilo_twoDarray_ptr);
//   cityimage.dynamic_colortable=false;
   ...
   xyzpfunc::write_xyzp_data(
      ...
      flat_filenamestr);
//   cityimage.writeimage("zcomplete_flat",
//      zcompletely_flattened_twoDarray_ptr);

// For subsequently algorithms, we copy the filtered and flattened z
// image back into *ztwoDarray_ptr:

   zlevel_twoDarray_ptr->copy(cityimage.get_z2Darray_ptr());
...
// asphalt regions:
   ...
   featurefunc::remove_feature_holes_from_feature_map(
      featurefunc::road_sentinel_value,refined_features_twoDarray_ptr,
      cityimage.get_z2Darray_ptr(),"asphalt",cityimage.get_imagedir());
   featurefunc::remove_feature_holes_from_feature_map(
      featurefunc::building_sentinel_value,refined_features_twoDarray_ptr,
      cityimage.get_z2Darray_ptr(),"building",cityimage.get_imagedir());
   featurefunc::remove_feature_holes_from_feature_map(
      featurefunc::grass_sentinel_value,refined_features_twoDarray_ptr,
...
// corresponding to sites within the buildings network:
   ...
   featurefunc::recolor_feature_pixels(
      featurefunc::building_sentinel_value,featurefunc::tree_sentinel_value,
      refined_features_twoDarray_ptr);
...
// in buildings network onto features map:
   ...
   featurefunc::recolor_feature_pixels(
      ...
      refined_features_twoDarray_ptr);
   cityimage.draw_building_site_pixels(refined_features_twoDarray_ptr);
   string final_features_filename=cityimage.get_imagedir()
      +"final_features.xyzp";
   xyzpfunc::write_xyzp_data(
      cityimage.get_z2Darray_ptr(),refined_features_twoDarray_ptr,
...
// output file.  The fused results look quite striking!
   ...
   cityimage.update_logfile("ground_bldgs");
}
