// ==========================================================================
// Program COMPARE_SURF_HISTOGRAMS imports sets of cumulative
...
//			  ./compare_SURF_histograms

// ==========================================================================
// Last updated on 11/8/13; 11/12/13; 11/26/13
// ==========================================================================
...
//   string JAV_subdir="/data/video/JAV/UIUC_Broadcast_News/";
   ...
   double min_SURF_scale_score=0.8;
   double min_SURF_angle_score=0.7;
   double min_SURF_separation_score=0.95;

//   double min_SURF_scale_score=0.8;
//   double min_SURF_angle_score=0.7;
//   double min_SURF_separation_score=0.7;
...
/*
   ...
   double min_SURF_scale_score=0.94;
   double min_SURF_angle_score=0.95;
   double min_SURF_separation_score=0.94;
*/
...
// within same video clips:
   ...
   {
      min_SURF_scale_score=min_SURF_angle_score=
         min_SURF_separation_score=0;
   }

   vector<double> min_SURF_scores;
   min_SURF_scores.push_back(min_SURF_scale_score);
   min_SURF_scores.push_back(min_SURF_angle_score);
   min_SURF_scores.push_back(min_SURF_separation_score);
...
// Move metafiles generated by program COMPUTE_SURF_HISTOGRAMS into
// scale/, angle/ and separation/ subdirectories of
// SURF_histograms_subdir/ :

   string SURF_scale_subdir=SURF_histograms_subdir+"scale/";
   string SURF_angle_subdir=SURF_histograms_subdir+"angle/";
   string SURF_separation_subdir=SURF_histograms_subdir+"separation/";
   filefunc::dircreate(SURF_scale_subdir);
   filefunc::dircreate(SURF_angle_subdir);
   filefunc::dircreate(SURF_separation_subdir);

   string unix_cmd="mv "+SURF_histograms_subdir+"*_scales_*.meta "+
      SURF_scale_subdir;
   sysfunc::unix_command(unix_cmd);
   unix_cmd="mv "+SURF_histograms_subdir+"*_angles_*.meta "+
      SURF_angle_subdir;
   sysfunc::unix_command(unix_cmd);
   unix_cmd="mv "+SURF_histograms_subdir+"*_separation_*.meta "+
      SURF_separation_subdir;
   sysfunc::unix_command(unix_cmd);

// Move metafiles within scale/, number_angle/ and separation
// subdirectories into angle/ and cumulative/ subsubdirectories:

   string SURF_dens_scale_subdir=SURF_scale_subdir+"density/";
   string SURF_dens_angle_subdir=SURF_angle_subdir+"density/";
   string SURF_dens_separation_subdir=SURF_separation_subdir+"density/";
   filefunc::dircreate(SURF_dens_scale_subdir);
   filefunc::dircreate(SURF_dens_angle_subdir);
   filefunc::dircreate(SURF_dens_separation_subdir);

   string SURF_cum_scale_subdir=SURF_scale_subdir+"cumulative/";
   string SURF_cum_angle_subdir=SURF_angle_subdir+"cumulative/";
   string SURF_cum_separation_subdir=SURF_separation_subdir+"cumulative/";
   filefunc::dircreate(SURF_cum_scale_subdir);
   filefunc::dircreate(SURF_cum_angle_subdir);
   filefunc::dircreate(SURF_cum_separation_subdir);

   unix_cmd="mv "+SURF_scale_subdir+"*_density.meta "+SURF_dens_scale_subdir;
   sysfunc::unix_command(unix_cmd);
   unix_cmd="mv "+SURF_scale_subdir+"*_cum.meta "+SURF_cum_scale_subdir;
   sysfunc::unix_command(unix_cmd);

   unix_cmd="mv "+SURF_angle_subdir+"*_density.meta "+SURF_dens_angle_subdir;
   sysfunc::unix_command(unix_cmd);
   unix_cmd="mv "+SURF_angle_subdir+"*_cum.meta "+SURF_cum_angle_subdir;
   sysfunc::unix_command(unix_cmd);

   unix_cmd="mv "+SURF_separation_subdir+"*_density.meta "+
      SURF_dens_separation_subdir;
   sysfunc::unix_command(unix_cmd);
   unix_cmd="mv "+SURF_separation_subdir+"*_cum.meta "+
      SURF_cum_separation_subdir;
   sysfunc::unix_command(unix_cmd);

   
   vector<string> SURF_cum_subdirs;
   SURF_cum_subdirs.push_back(SURF_cum_scale_subdir);
   SURF_cum_subdirs.push_back(SURF_cum_angle_subdir);
   SURF_cum_subdirs.push_back(SURF_cum_separation_subdir);
   
   vector<string> allowed_suffixes;
   allowed_suffixes.push_back("meta");

// Loop over index e labeling SURF feature type starts here:

   string cum_meta_substring="_cum.meta";
   for (int e=0; e<SURF_cum_subdirs.size(); e++)
   {
      outputfunc::print_elapsed_time();
      vector<string> meta_filenames=
         filefunc::files_in_subdir_matching_substring(
            SURF_cum_subdirs[e],cum_meta_substring);
      int n_metafiles=meta_filenames.size();

      cout << "e = " << e
           << " SURF_cum_subdirs[e] = " 
           << SURF_cum_subdirs[e] << endl;
      cout << "n_metafiles = " << n_metafiles << endl;      

// Import cumulative distributions generated by program
// COMPUTE_SURF_HISTOGRAMS:

      string banner="Importing SURF metafiles from "+SURF_cum_subdirs[e];
...
//         cout << column_values[0] << "  " << column_values[1] << endl;
      ...
      cout << endl;
      cout << "Computing SURF cumulative distribution scores:" << endl;
      cout << endl;
...
//              << max_cum_difference << endl;
            ...
            SURF_scores.push_back(1-max_cum_difference);
      ...
      prob_distribution prob(SURF_scores,100,0);
      prob.set_densityfilenamestr(
         SURF_cum_subdirs[e]+"SURF_density_scores.meta");
      prob.set_cumulativefilenamestr(
         SURF_cum_subdirs[e]+"SURF_cum_scores.meta");
      prob.writeprobdists(false);

      templatefunc::Quicksort_descending(
         SURF_scores,basename_i_indices,basename_j_indices);

      string output_filename=SURF_cum_subdirs[e];
      if (different_clips_flag)
      {
         output_filename += "image_SURFs.comparison";
      }
      else
      {
         output_filename += "image_SURFs_sameclips.comparison";
      }
      ...
      for (int t=0; t<SURF_scores.size(); t++)
      {
         if (SURF_scores[t] < min_SURF_scores[e]) continue;
...
//          (frame_ID_j==frame_ID_i+1) && 
         ...
         {
            outstream << SURF_scores[t] << "   "
                      << image_pair_names << endl;
         }
      } // loop over index t labeling SURF scores

      filefunc::closefile(output_filename,outstream);
      banner="Exported image SURF comparison to "+output_filename;
      outputfunc::write_big_banner(banner);

   } // loop over index e labeling SURF cumulative subdirectories

   cout << "At end of program COMPARE_SURF_HISTOGRAMS" << endl;
   outputfunc::print_elapsed_time();
}
