// ==========================================================================
// Program GENERATE_NONMATCHING_FILENAMES scans over montage images
// within a specified input subdirectory.  It extracts clip and frame
// IDs for nonmatching image pairs.  Full path filenames for
// nonmatching image pairs are then exported to an output text file.

//		      ./generate_nonmatching_filenames
...
// Import basic HOG BoW processing parameters:
   ...
   string JAV_subdir,input_mismatches_subdir;
...
//         input_mismatches_subdir="~/Desktop/Newswrap_keyframe_mismatches/";
         input_mismatches_subdir=filefunc::text_line[13];
      }
...
//         input_mismatches_subdir="~/Desktop/BB_keyframe_mismatches/";
         input_mismatches_subdir=filefunc::text_line[14];
      }
      else
      {
         exit(-1);
...
//      input_mismatches_subdir="~/Desktop/tidmarsh_mismatches/";
      input_mismatches_subdir=filefunc::text_line[15];
   ...
   vector<string> annotated_pair_filenames=filefunc::image_files_in_subdir(
      input_mismatches_subdir);
   int n_nonmatching_pairs=annotated_pair_filenames.size();

   string nonmatching_images_filename=root_subdir+
      stringfunc::number_to_string(n_nonmatching_pairs)+
      "_nonmatching_image_filenames.dat";
   ofstream outstream;
   filefunc::openfile(nonmatching_images_filename,outstream);

   string separator_chars="_.";
   for (int i=0; i<n_nonmatching_pairs; i++)
   {
...
//      outputfunc::enter_continue_char();
   ...
   } // loop over index i labeling nonmatching image pairs
   filefunc::closefile(nonmatching_images_filename,outstream);

   string banner="Exported nonmatching image pair filenames to "+
      nonmatching_images_filename;
   outputfunc::write_big_banner(banner);

   cout << "At end of program GENERATE_NONMATCHING_FILENAMES" << endl;
