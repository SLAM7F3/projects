// ==========================================================================
// Program COMPARE_ELLIPSE_HISTOGRAMS imports sets of cumulative
// probability distribution functions generated by program
// COMPUTE_ELLIPSE_HISTOGRAMS.  Looping over all imported cumulative
// distributions, it computes the maximum difference between
// successive pairs.  The similarity between two cumulative
// distributions is taken to equal 1 - their maximum difference.  The
// similarity scores are sorted in descending order and exported to an
// output text file with the corresponding pair's image filenames.

//			  ./compare_ellipse_histograms

// ==========================================================================
// Last updated on 10/23/13; 10/24/13; 10/28/13; 11/8/13
// ==========================================================================

#include <cstdlib>
#include <iostream>
#include <map>
#include <set>
#include <string>
#include <vector>

#include "general/filefuncs.h"
#include "math/ltduple.h"
#include "templates/mytemplates.h"
#include "general/outputfuncs.h"
#include "filter/piecewise_linear.h"
#include "math/prob_distribution.h"
#include "general/stringfuncs.h"
#include "general/sysfuncs.h"
#include "time/timefuncs.h"

using std::cin;
using std::cout;
using std::endl;
using std::ios;
using std::map;
using std::ofstream;
using std::pair;
using std::string;
using std::vector;

// ==========================================================================
int main(int argc, char *argv[])
// ==========================================================================
{
   std::set_new_handler(sysfunc::out_of_memory);
   timefunc::initialize_timeofday_clock();      

   bool keyframes_flag=true;
   bool different_clips_flag=false;

   string input_char;
   cout << "Enter 'k' to process just video keyframes:" << endl;
   cin >> input_char;
   if (input_char != "k")
   {
      keyframes_flag=false;
   }
   cout << "Enter 'w' to compare histograms within and not between clips:"
        << endl;
   cin >> input_char;
   if (input_char != "w")
   {
      different_clips_flag=true;
   }

//   string JAV_subdir="/data/video/JAV/NewsWraps/early_Sep_2013/";
   string JAV_subdir="/data/video/JAV/NewsWraps/w_transcripts/";
//   string JAV_subdir="/data/video/JAV/UIUC_Broadcast_News/";
   string root_subdir=JAV_subdir;
   string images_subdir=root_subdir+"jpg_frames/";
   string keyframes_subdir=images_subdir+"keyframes/";

   string ellipses_subdir=root_subdir+"ellipses/";
   string ellipse_metafiles_subdir=ellipses_subdir+"metafiles/";
//   double min_ellipse_area_score=0.92;
//   double min_ellipse_density_score=0.89;
//   double min_ellipse_separation_score=0.94;

   double min_ellipse_area_score=0.8;
   double min_ellipse_density_score=0.7;
   double min_ellipse_separation_score=0.7;

/*
   string ImageEngine_subdir="/data/ImageEngine/";
   string tidmarsh_subdir=ImageEngine_subdir+"tidmarsh/";
   string images_subdir=tidmarsh_subdir;
   string root_subdir=tidmarsh_subdir;
   string ellipses_subdir=root_subdir+"ellipses/";
   string ellipse_metafiles_subdir=ellipses_subdir+"metafiles/";
   double min_ellipse_area_score=0.94;
   double min_ellipse_density_score=0.95;
   double min_ellipse_separation_score=0.94;
*/

// Report all ellipse matching scores when we're comparing keyframes
// within same video clips:

   if (keyframes_flag && !different_clips_flag)
   {
      min_ellipse_area_score=min_ellipse_density_score=
         min_ellipse_separation_score=0;
   }

   vector<double> min_ellipse_scores;
   min_ellipse_scores.push_back(min_ellipse_area_score);
   min_ellipse_scores.push_back(min_ellipse_density_score);
   min_ellipse_scores.push_back(min_ellipse_separation_score);

// Store keyframes' clip and frame IDs within STL map keyframes_map:

   typedef map<DUPLE,string> KEYFRAMES_MAP;
   KEYFRAMES_MAP keyframes_map;

// independent DUPLE holds video image clip and frame ID
// dependent string holds video image's filename

   if (keyframes_flag)
   {
      vector<string> keyframe_filenames=filefunc::image_files_in_subdir(
         keyframes_subdir);

      for (int i=0; i<keyframe_filenames.size(); i++)
      {
         vector<string> substrings=
            stringfunc::decompose_string_into_substrings(
               filefunc::getbasename(keyframe_filenames[i])," _-");
//         for (int s=0; s<substrings.size(); s++)
//         {
//            cout << "i = " << i << " s = " << s << " substring[s] = " 
//                 << substrings[s] << endl;
//         }

// If input filename has form clip_0000_frame-00001.jpg, record clip's
// ID within STL vector:

         int clip_ID=-1;
         int frame_ID=-1;
         if (substrings.size()==4)
         {
            if (substrings[0]=="clip")
            {
               clip_ID=stringfunc::string_to_number(substrings[1]);
               frame_ID=stringfunc::string_to_number(substrings[3]);
               keyframes_map[DUPLE(clip_ID,frame_ID)]=keyframe_filenames[i];
            }
         }
      } // loop over index i labeling keyframe filenames
      
   } // keyframes_flag conditional
//   cout << "keyframes_map.size() = " << keyframes_map.size() << endl;

// Move metafiles generated by program COMPUTE_ELLIPSE_HISTOGRAMS into
// area/, number_density/ and separation/ subdirectories of
// ellipse_metafiles_subdir/ :

   string ellipse_area_subdir=ellipse_metafiles_subdir+"area/";
   string ellipse_numberdensity_subdir=
      ellipse_metafiles_subdir+"number_density/";
   string ellipse_separation_subdir=ellipse_metafiles_subdir+"separation/";
   filefunc::dircreate(ellipse_area_subdir);
   filefunc::dircreate(ellipse_numberdensity_subdir);
   filefunc::dircreate(ellipse_separation_subdir);

   string unix_cmd="mv "+ellipse_metafiles_subdir+"*_area_*.meta "+
      ellipse_area_subdir;
   sysfunc::unix_command(unix_cmd);
   unix_cmd="mv "+ellipse_metafiles_subdir+"*_numberdens_*.meta "+
      ellipse_numberdensity_subdir;
   sysfunc::unix_command(unix_cmd);
   unix_cmd="mv "+ellipse_metafiles_subdir+"*_separation_*.meta "+
      ellipse_separation_subdir;
   sysfunc::unix_command(unix_cmd);

// Move metafiles within area/, number_density/ and separation
// subdirectories into density/ and cumulative/ subsubdirectories:

   string ellipse_dens_area_subdir=ellipse_area_subdir+"density/";
   string ellipse_dens_numberdensity_subdir=
      ellipse_numberdensity_subdir+"density/";
   string ellipse_dens_separation_subdir=
      ellipse_separation_subdir+"density/";
   filefunc::dircreate(ellipse_dens_area_subdir);
   filefunc::dircreate(ellipse_dens_numberdensity_subdir);
   filefunc::dircreate(ellipse_dens_separation_subdir);

   string ellipse_cum_area_subdir=ellipse_area_subdir+"cumulative/";
   string ellipse_cum_numberdensity_subdir=
      ellipse_numberdensity_subdir+"cumulative/";
   string ellipse_cum_separation_subdir=
      ellipse_separation_subdir+"cumulative/";
   filefunc::dircreate(ellipse_cum_area_subdir);
   filefunc::dircreate(ellipse_cum_numberdensity_subdir);
   filefunc::dircreate(ellipse_cum_separation_subdir);

   unix_cmd="mv "+ellipse_area_subdir+"*_density.meta "+
      ellipse_dens_area_subdir;
   sysfunc::unix_command(unix_cmd);
   unix_cmd="mv "+ellipse_area_subdir+"*_cum.meta "+
      ellipse_cum_area_subdir;
   sysfunc::unix_command(unix_cmd);

   unix_cmd="mv "+ellipse_numberdensity_subdir+"*_density.meta "+
      ellipse_dens_numberdensity_subdir;
   sysfunc::unix_command(unix_cmd);
   unix_cmd="mv "+ellipse_numberdensity_subdir+"*_cum.meta "+
      ellipse_cum_numberdensity_subdir;
   sysfunc::unix_command(unix_cmd);

   unix_cmd="mv "+ellipse_separation_subdir+"*_density.meta "+
      ellipse_dens_separation_subdir;
   sysfunc::unix_command(unix_cmd);
   unix_cmd="mv "+ellipse_separation_subdir+"*_cum.meta "+
      ellipse_cum_separation_subdir;
   sysfunc::unix_command(unix_cmd);

   
   vector<string> ellipse_cum_subdirs;
   ellipse_cum_subdirs.push_back(ellipse_cum_area_subdir);
   ellipse_cum_subdirs.push_back(ellipse_cum_numberdensity_subdir);
   ellipse_cum_subdirs.push_back(ellipse_cum_separation_subdir);
   
   vector<string> allowed_suffixes;
   allowed_suffixes.push_back("meta");

// Loop over index e labeling ellipse feature type starts here:

   string cum_meta_substring="_cum.meta";
   for (int e=0; e<ellipse_cum_subdirs.size(); e++)
   {
      outputfunc::print_elapsed_time();
      vector<string> meta_filenames=
         filefunc::files_in_subdir_matching_substring(
            ellipse_cum_subdirs[e],cum_meta_substring);
      int n_metafiles=meta_filenames.size();

      cout << "e = " << e
           << " ellipse_cum_subdirs[e] = " 
           << ellipse_cum_subdirs[e] << endl;
      cout << "n_metafiles = " << n_metafiles << endl;      

// Import cumulative distributions generated by program
// COMPUTE_ELLIPSE_HISTOGRAMS:

      string banner="Importing ellipse metafiles from "+ellipse_cum_subdirs[e];
      outputfunc::write_banner(banner);

      typedef pair< vector<double> , vector<double> > CUM_DISTRIBUTION;
      vector<CUM_DISTRIBUTION> cum_distributions;

      vector<short unsigned int> basename_i_indices,basename_j_indices;
      vector<float> ellipse_scores;
      basename_i_indices.reserve(0.5*sqr(n_metafiles));
      basename_j_indices.reserve(0.5*sqr(n_metafiles));
      ellipse_scores.reserve(0.5*sqr(n_metafiles));

      vector<string> hist_filenames;

      for (int i=0; i<n_metafiles; i++)
      {
         outputfunc::update_progress_fraction(i,100,n_metafiles);

         if (keyframes_flag)
         {
            string basename=filefunc::getbasename(meta_filenames[i]);
            vector<string> substrings=
               stringfunc::decompose_string_into_substrings(
                  stringfunc::prefix(basename)," _-");
//            for (int s=0; s<substrings.size(); s++)
//            {
//               cout << " s = " << s << " substring[s] = " 
//                    << substrings[s] << endl;
//            }

// If input filename has form clip_0000_frame-00001, record clip's
// ID within STL vector:

            int clip_ID=-1;
            int frame_ID=-1;
            if (substrings.size()==7)
            {
               if (substrings[0]=="clip")
               {
                  clip_ID=stringfunc::string_to_number(substrings[1]);
                  frame_ID=stringfunc::string_to_number(substrings[3]);
                  KEYFRAMES_MAP::iterator keyframe_iter=keyframes_map.find(
                     DUPLE(clip_ID,frame_ID));
                  if (keyframe_iter==keyframes_map.end())
                  {
                     continue;
                  }
//               else
//               {
//                  cout << "clip_ID = " << clip_ID << " frame_ID = " << frame_//ID
//                       << " is a keyframe" << endl;
//               }
               }
            } // substrings.size() conditional
         } // keyframes_flag conditional

         filefunc::ReadInfile(meta_filenames[i]);

         CUM_DISTRIBUTION curr_cum_distribution;
         for (int linenumber=17; linenumber < filefunc::text_line.size(); 
              linenumber++)
         {
            vector<double> column_values=stringfunc::string_to_numbers(
               filefunc::text_line[linenumber]);
//         cout << column_values[0] << "  " << column_values[1] << endl;
            curr_cum_distribution.first.push_back(column_values[0]);
            curr_cum_distribution.second.push_back(column_values[1]);
         }
         cum_distributions.push_back(curr_cum_distribution);
         hist_filenames.push_back(meta_filenames[i]);

      } // loop over index i labeling input metafiles
      cout << endl;

      cout << "cum_distributions.size() = "
           << cum_distributions.size() << endl;

      cout << endl;
      cout << "Computing ellipse cumulative distribution scores:" << endl;
      cout << endl;

      outputfunc::print_elapsed_time();

      int i_max=cum_distributions.size();
      int j_max=i_max;
      for (int i=0; i<i_max-1; i++)
      {
         outputfunc::update_progress_fraction(i,10,n_metafiles);
         CUM_DISTRIBUTION* curr_cum_distribution_ptr=
            &(cum_distributions[i]);
         piecewise_linear pl(curr_cum_distribution_ptr->first,
                             curr_cum_distribution_ptr->second);
      
//         if (!different_clips_flag) j_max=basic_math::min(i+2,i_max);

         for (int j=i+1; j<j_max; j++)
         {
            CUM_DISTRIBUTION* next_cum_distribution_ptr=
               &(cum_distributions[j]);
            
// Search for maximum discrepancy between curr_cum_distribution and
// next_cum_distribution:

            double max_cum_difference=-1;
            for (int b=0; b<next_cum_distribution_ptr->first.size(); b++)
            {
               double next_x=next_cum_distribution_ptr->first[b];
               double next_cum=next_cum_distribution_ptr->second[b];
               double curr_cum=pl.value(next_x);
               max_cum_difference=basic_math::max(
                  max_cum_difference,fabs(next_cum-curr_cum));
            }

//         cout << "i = " << i << " j = " << j << " max_cum_difference = "
//              << max_cum_difference << endl;

            ellipse_scores.push_back(1-max_cum_difference);
//            ellipse_scores.push_back(max_cum_difference);
            basename_i_indices.push_back(i);
            basename_j_indices.push_back(j);

         } // loop over index j 
      
      } // loop over index i labeling input metafiles

      prob_distribution prob(ellipse_scores,100,0);
      prob.set_densityfilenamestr(
         ellipse_cum_subdirs[e]+"ellipse_density_scores.meta");
      prob.set_cumulativefilenamestr(
         ellipse_cum_subdirs[e]+"ellipse_cum_scores.meta");
      prob.writeprobdists(false);
//      unix_cmd="mv prob_density.jpg ellipse_scores.jpg";
//      sysfunc::unix_command(unix_cmd);

      templatefunc::Quicksort_descending(
         ellipse_scores,basename_i_indices,basename_j_indices);

      string output_filename=ellipse_cum_subdirs[e];
      if (different_clips_flag)
      {
         output_filename += "image_ellipses.comparison";
      }
      else
      {
         output_filename += "image_ellipses_sameclips.comparison";
      }

      if (keyframes_flag)
      {
         output_filename += ".keyframes";
      }

      ofstream outstream;
      filefunc::openfile(output_filename,outstream);
      outstream << "# Score    Image_prefix   Image'_prefix   " 
                << endl << endl;

      for (int t=0; t<ellipse_scores.size(); t++)
      {
         if (ellipse_scores[t] < min_ellipse_scores[e]) continue;

         string basename_i=filefunc::getbasename(
            hist_filenames[basename_i_indices[t]]);
         string basename_j=filefunc::getbasename(
            hist_filenames[basename_j_indices[t]]);
//         cout << "basename_i = " << basename_i
//              << " basename_j = " << basename_j << endl;

         int posn=stringfunc::first_substring_location(
            basename_i,"_ellipse");
         basename_i=basename_i.substr(0,posn);
         posn=stringfunc::first_substring_location(
            basename_j,"_ellipse");
         basename_j=basename_j.substr(0,posn);

//         cout << "basename_i = " << basename_i
//              << " basename_j = " << basename_j << endl;

         string image_pair_names=
            stringfunc::prefix(basename_i)+"  "+
            stringfunc::prefix(basename_j);
//      cout << "t = " << t
//           << " image_pair_names = " << image_pair_names << endl;


         vector<string> substrings=stringfunc::decompose_string_into_substrings(
            image_pair_names," _-");

         int clip_ID_i=-1;
         int clip_ID_j=-2;
         int frame_ID_i,frame_ID_j;
         if (substrings.size()==8)
         {
            if (substrings[0]=="clip")
            {
               clip_ID_i=stringfunc::string_to_number(substrings[1]);
               frame_ID_i=stringfunc::string_to_number(substrings[3]);
               clip_ID_j=stringfunc::string_to_number(substrings[5]);
               frame_ID_j=stringfunc::string_to_number(substrings[7]);
//            cout << "clip_ID_i = " << clip_ID_i
//                 << " clip_ID_j = " << clip_ID_j
//                 << endl;
            }
         } // substrings.size()==8 conditional

         if ( ((clip_ID_i != clip_ID_j) && different_clips_flag) ||
         ((clip_ID_i==clip_ID_j) && 
//          (frame_ID_j==frame_ID_i+1) && 
         !different_clips_flag) )
         {
            outstream << ellipse_scores[t] << "   "
                      << image_pair_names << endl;
         }
      } // loop over index t labeling ellipse scores

      filefunc::closefile(output_filename,outstream);
      banner="Exported image ellipse comparison to "+output_filename;
      outputfunc::write_big_banner(banner);

   } // loop over index e labeling ellipse cumulative subdirectories

   cout << "At end of program COMPARE_ELLIPSE_HISTOGRAMS" << endl;
   outputfunc::print_elapsed_time();
}

