// =======================================================================
// Program UCORRECT_PANOS first imports a set of subsampled, 
// "V-corrected" WISP panoramas separated in time generated by program
// VCORRECT_PANOS.  It extracts SIFT and ASIFT features via calls to
...
// following Lowe's conventions. UCORRECT_PANOS performs tiepoint
// matching via homography estimation and RANSAC.

// Each tiepoint pair between a 0th and later WISP frame establishes a
// correspondence between some horizontal "X" and and counterpart 
// "U" coordinate.  Define Delta=U-X.  We fit a periodic function of
// the form

// Delta = U-X = avg_Delta + beta sin(2 PI x/Umax+phi)
//		+ gamma sin(4 PI x/Umax+phi)

// to all tiepoint pairs between the 0th and later WISP frames.  A
// brute-force search is conducted for parameters beta, gamma and phi.
// The fitted function for Delta is subsequently used to "un-U-warp"
// the input "V-corrected" panorama.

// UCORRECT_PANOS exports a set of full-resolution WISP panoramas
// which should (theoretically) be stabilized relative to the 0th
// frame's V-corrected panorama.
...
double cosusoid(double u,double v,double v_avg,double A,double phi,double Umax)
{
   double arg=2*PI*u/Umax+phi;
   double shear_du=A*(v-v_avg)*cos(arg);
   return shear_du;
}
...
// Match SIFT & ASIFT features across image pairs:
      ...
      for (int r=0; r<n_inliers; r++)
      {
         ...
         Y.push_back(curr_XY.get(1)); // zeroth photo
         U.push_back(curr_UV.get(0)); // later photo
         V.push_back(curr_UV.get(1)); // later photo
...
//         cout << "beta_stop= " << beta_stop << endl;
      param_range beta(beta_start,beta_stop,31);

      double phi;
      cout << "Enter phi in degrees:" << endl;
      cin >> phi;
      phi *= PI/180;

      double v_avg=0.492548;

/*
      int n_iters=50;
      double min_avg_abs_residual=POSITIVEINFINITY;
...
//               double fit_func=avg_Delta;
               double fit_func=
                  avg_Delta+
                  cosusoid(curr_X,curr_Y,v_avg,beta.get_value(),phi,Umax);
               double curr_residual=curr_Delta-fit_func;
               avg_abs_residual += fabs(curr_residual);
            } // loop over index j labeling Delta measurements
                  
            avg_abs_residual /= Delta.size();

            if (avg_abs_residual < min_avg_abs_residual)
            {
               min_avg_abs_residual=avg_abs_residual;
               beta.set_best_value();
               cout << "min_avg_abs_residual = " << min_avg_abs_residual
      ...
      cout << endl;
      cout << "min_avg_abs_residual = " << min_avg_abs_residual << endl;
