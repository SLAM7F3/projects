// ========================================================================
// Program WIICITIES is a variant of BUNDLECITIES which displays
// reconstructed OBSFRUSTA generated via Noah Snavely's BUNDLER program.
// WIICITIES was developed to be manipulated via a Wiimote for Family
// Day 2011 demo purposes.
// ========================================================================
// Last updated on 9/6/11; 9/9/11; 9/11/11
// ========================================================================
...
// Add a custom manipulator to the event handler list:
   ...
   CM_3D_ptr->set_min_camera_height_above_grid(200);	// meters
...
// Instantiate signposts and features decoration groups:
...
// Instantiate OBSFRUSTA decoration group:
   ...
   OBSFRUSTAGROUP_ptr->set_alpha_vary_selected_image_flag(true);
...
/*
   MovieKeyHandler* MoviesKeyHandler_ptr=
      new MovieKeyHandler(ModeController_ptr,
...
// Instantiate ArrowHUD to hold left/right OBSFRUSTUM selection
// arrows:

   ArrowHUD* ArrowHUD_ptr=new ArrowHUD(colorfunc::purple);
   ArrowHUD_ptr->set_N_nodemask(0);
   ArrowHUD_ptr->set_E_nodemask(0);
   ArrowHUD_ptr->set_S_nodemask(0);
   ArrowHUD_ptr->set_W_nodemask(0);
   root->addChild(ArrowHUD_ptr->getProjection());
   OBSFRUSTAGROUP_ptr->set_ArrowHUD_ptr(ArrowHUD_ptr);
...
// FAKE FAKE:  Friday, Sep 9, 2011 at 1 pm
// Experiment with allowing users to move to left or right OBSFRUSTA
// for Statue of Liberty photos:

   OBSFRUSTAGROUP_ptr->read_circularly_ordered_OBSFRUSTA_IDs();


   WM.set_CM_3D_ptr(CM_3D_ptr);
   WM.set_PointCloudsGroup_ptr(&clouds_group);
   WM.set_OBSFRUSTAGROUP_ptr(OBSFRUSTAGROUP_ptr);

   const int wii_iters=4;
   while( !window_mgr_ptr->done() )
   {
      for (int w=0; w<wii_iters; w++)
      {
         WM.update_state();
         WM.check_for_NYC_1Kdemo_control_input();
      }
