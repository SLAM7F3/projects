...
// =======================================================================
// Last updated on 5/9/13; 5/10/13; 5/13/13; 5/14/13
// =======================================================================
...
int main( int argc, char** argv ) 
{
   ...
   double f_puma=-2.36382;	// FOV_u=31.5 degs, FOV_v=23.886 degs
//   double f_puma=-4.23307;	// FOV_u=17.9 degs, FOV_v=13.4728 degs
//   double f_puma=-7.0526;	// FOV_u=10.8 degs, FOV_v=8.1105 degs
//   double f_puma=-11.3891;	// FOV_u=6.7 degs, FOV_v=5.02751 degs
...
// Create softlinks between key files in all_keys_subdir with
// images_subdir:

   string unix_cmd="ln -s "+all_keys_subdir+"*.key "+images_subdir;
...
// FAKE FAKE:  Tues May 14, 2013 at 3:38 pm
// Hardwire focal length for Puma UAV video camera:

      camera_ptr->set_f(f_puma);
...
// Export raw inlier triangulated points to TDP file:
...
// Delete any features whose 3D world points lie too far from ground plane:
   ...
   int n_outliers=0;
   vector<int> feature_ID_to_destroy;
   for (int f=0; f<n_features; f++)
   {
      Feature* Feature_ptr=FeaturesGroup_ptr->get_Feature_ptr(f);
      double curr_t=FeaturesGroup_ptr->get_curr_t();
      ...
      if (inlier_points_iter==inlier_points_map.end())
      {
//         cout << "Feature ID = " << Feature_ptr->get_ID() << " is an outlier"
//              << endl;
         feature_ID_to_destroy.push_back(Feature_ptr->get_ID());
   ...
   cout << "feature_ID_to_destroy.size() = "
        << feature_ID_to_destroy.size() << endl;
   for (int f=0; f<feature_ID_to_destroy.size(); f++)
   {
      FeaturesGroup_ptr->destroy_feature(feature_ID_to_destroy[f]);
   }
   n_features=FeaturesGroup_ptr->get_n_Graphicals();
   cout << "After removing ground-plane outliers, n_features = " 
        << n_features << endl;
...
/*
   ...
   FeaturesGroup_ptr->write_feature_html_file(
      ...
      output_3D_rays_flag);
   unix_cmd="mv photograph_features.html "+bundler_IO_subdir;
   sysfunc::unix_command(unix_cmd);
*/
...
// Note added on 4/26/13:  

// 2.  bundler/images needs to have links to all_keys/foo.key files
...
// ./bundler.  So generate modified version of list_tmp.txt file:
   ...
   unix_cmd="/home/cho/software/bundler-v0.4-source/bin/bundler ";
