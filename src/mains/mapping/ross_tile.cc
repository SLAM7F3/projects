// =======================================================================
// Ross' tile program modified to compile within our source tree
// =======================================================================
// Last updated on 5/10/07; 6/25/07; 2/13/09; 12/22/09
// =======================================================================
...
class TdpDataType 
{
  public:
   ...
   TdpDataType( unsigned int k, string n, TypeEnumeration t ) :
      valid(true),
      key(k),
      name(n),
      type(t)
...
// -----------------------------------------------------------------------
/** Abstract base class for tiles of different types. */
class Tile {
  public:
...
class TileBox : public Tile
{
  public:
   ...
   bool contains( const osg::Vec3f& point ) 
      {
//         cout << "inside TileBox::contains()" << endl;
//         cout << "xmin = " << bounds.xMin() << " xmax = " << bounds.xMax()
//              << endl;
//         cout << "ymin = " << bounds.yMin() << " ymax = " << bounds.yMax()
//              << endl;
  ...
  public:
   ...
   TileCircle( const osg::Vec2d& center, const double radius ) :
      ...
      _radius2( radius*radius )
      {
         ...
         bounds.set( osg::Vec3d( center-delta, -FLT_MAX ), 
                     osg::Vec3d( center+delta, FLT_MAX ) );
      }
...
// Tests if this tile's circle contains the 3D point in X and Y only.
   ...
   bool contains( const osg::Vec3f& point ) {
      return ( osg::Vec2d( point.x(), point.y() ) - _center ).length2() 
         < _radius2;
   }
...
int main (int argc, char * argv[])
{
   ...
   arguments.getApplicationUsage()->addCommandLineOption( "--ll", "Specify coordinates in lat/long (vs. UTM)" );
       ...
       arguments.argc()<=1)
   {
      ...
      cout << endl;
      cout << "Sample: tile -o tile_output_dir -b boston -n 8 4 input_dir/*.tdp" << endl;
      cout << endl;
      cout << "When using the crop or poly mode, you must specify coordinates relative to " << endl;
      cout << "the local UTM zone, not in any offset space. Or, specify in global lat/long and " << endl;
      cout << "use the --ll flag. Polygons are described by a whitespace delimited text file, in " << endl;
      cout << "East/North/Alt triplets in UTM or Long/Lat/Alt triplets with --ll. These files " << endl;
      cout << "can also be used with the marker tool. The altitudes given in this file are ignored." << endl;
      cout << endl;
      cout << "The cropping modes --crop-z and --crop-meta are independent of the output tiles. Points" << endl;
      cout << "falling outside the given crop ranges are filtered out before being written to a tile." << endl;
      cout << endl;
   ...
   string crop_poly_filename;
   float alignment = 0.0f;
   float crop_z_min=NEGATIVEINFINITY;
   float crop_z_max=POSITIVEINFINITY;
   ...
   bool	crop_by_box = arguments.read( 
      "--crop", crop_box_range[0].x(), crop_box_range[0].y(), 
      crop_box_range[1].x(), crop_box_range[1].y() );
   ...
   if ( ( (int)calc_tile_size + calc_tile_num + crop_by_box + crop_by_poly + crop_by_circle ) != 1 ) {
      ...
      cout << "You must specify exactly one argument from the group -s, -n, --crop, --circle, or --poly. They are mutually exclusive." << endl;
      cout << endl;
      return 1;
   }

// Quiet the TDP library, because we will run into warnings under
// correct operation
   ...
   for (unsigned int file_index=0; file_index<file_info_list.size(); 
        file_index++)
   {
      FileInfo&	file_info = file_info_list[file_index];
      string file_name = file_info.path;
      ...
      if ( tdp_contents[ TdpKeyXYZ_POINT_DATA ] < 1 && 
           tdp_contents[ TdpKeyALIRT_XYZP ] < 1 ) {
         cerr << "The input file \"" << file_name 
                   << "\" does not contain any point cloud data. Skipping...\n";
      ...
      if ( tdp_contents[ TdpKeyUTMZone ] ) {
         input_file.klv_read( TdpKeyUTMZone, 0, 
                              (char *)utm_zone, 3 * sizeof(char) );
         utm_zone[3] = 0;
			
         osg::notify(osg::NOTICE) << "\tUTM Zone: " << utm_zone << endl;
      }
      ...
      if ( tdp_contents[ TdpKeyALIRT_THETA ] && !heading_align_tiles ) {
         ...
         osg::notify(osg::NOTICE) << "\tTheta Rotation: " << theta << " radians" << endl;
      }
      ...
      if ( tdp_contents[ TdpKeyUTMOffset ] ) {
         ...
         osg::notify(osg::NOTICE) << "\tUTM Zone Offset: " << offset << " meters" << endl;
      }
      ...
      if ( tdp_contents[ TdpKeyALIRT_MIN ] && tdp_contents[ TdpKeyALIRT_MAX ] ) {		
         ...
         {
            cout << "*** Warning: TdpKeyALIRT_MIN or TdpKeyALIRT_MAX contains a nearly zero value. This could indicate that it was incorrectly calculated." << endl;
            s_ZeroWarning = true;
         }
         ...
         if ( ! local_bounds.valid() )
            cerr << "Found illegal bounds in \"" << file_name << "\". I will ignoring this.\n";
      }
      ...
      if ( ! local_bounds.valid() ) {
         cerr << "The input file \"" << file_name << "\" does not contain a valid set of points. Skipping...\n";
      ...
      file_info.unified_extent = model::transform( 
         local_bounds, file_info.local_to_unified );
      bounds.expandBy( file_info.unified_extent );
   }
	
   if ( ! bounds.valid() ) {
      cerr << "There does not appear to be any valid data here.\n";
      return 1;
   }

   cout << "Bounds of input set: ";
   cout << "[" << bounds.xMin() << " " << bounds.xMax() << "] ";
   cout << "[" << bounds.yMin() << " " << bounds.yMax() << "] ";
   cout << "[" << bounds.zMin() << " " << bounds.zMax() << "]\n";
   ...
   double		unifiedAngle;
   {
      ...
      cout << "Unified Offset: " << unifiedOffset << "\n";
      cout << "Unified Angle: " << unifiedAngle << "\n";
   }
   ...
   osg::Matrixd	world_to_unified = osg::Matrixd::inverse( 
      unified.getLocalOffset() );
...
*/
         ...
         crop_box_range[0].x()=UTM_easting;
         crop_box_range[0].y()=UTM_northing;

         latlongfunc::LLtoUTM(
            crop_box_range[1].y(),crop_box_range[1].x(),
            UTM_zonenumber,northern_hemisphere_flag,
            UTM_northing,UTM_easting);
         crop_box_range[1].x()=UTM_easting;
         crop_box_range[1].y()=UTM_northing;
      } // convert_to_utm conditional
      ...
      cout << "Crop box converted into offset UTM:" << endl;
      cout << "Xmin: " << crop_box.xMin() << " Ymin: " << crop_box.yMin() 
                << " Xmax: " << crop_box.xMax() << " Ymax: " << crop_box.yMax() << endl;
      cout << endl;
      ...
      if ( ! input_file ) {
         cerr << "Can't open poly file \"" << crop_poly_filename << "\"\n";
         return 1;
      }
...
//            ll_to_utm( 23, vertex.y(), vertex.x(), vertex.y(), vertex.x(), UTMZone);
            ...
            latlongfunc::LLtoUTM(
               ...
               UTM_zonenumber,northern_hemisphere_flag,
               UTM_northing,UTM_easting);
            vertex.x()=UTM_easting;
            vertex.y()=UTM_northing;
         }
...
//         ll_to_utm( 23, circle_center.y(), circle_center.x(), circle_center.y(), circle_center.x(), UTMZone);
         ...
         latlongfunc::LLtoUTM(
            ...
            UTM_zonenumber,northern_hemisphere_flag,
            UTM_northing,UTM_easting);
         circle_center.x()=UTM_easting;
         circle_center.y()=UTM_northing;
      }
      ...
      // display crop box in local coords
      cout << "Crop circle converted into offset UTM:" << endl;
      cout << "X: " << circle_center.x() << " Y: " << circle_center.y() 
                << " Radius: " << circle_radius << endl;
      cout << endl;
   ...
   else
   {
      ...
      if ( calc_tile_size ) {
         ...
         tile_size.set( bounds.xMax() - bounds.xMin(), 
                        bounds.yMax() - bounds.yMin() );
         x_tiles = 1;
         y_tiles = 1;
      }

      cout << "Sorting " << file_info_list.size() << " input files into " 
           << x_tiles << " x " << y_tiles << " tiles." << endl;
      cout << "Tile size: " << tile_size.x() << " x " << tile_size.y() 
           << " m" << endl;	
   ...
   for( tileIter = tiles.begin(); tileIter != tiles.end(); ++tileIter ) {
      ...
      if( remove( (*tileIter)->filename.c_str() ) == 0 ) {
         cout << "Deleted \"" << (*tileIter)->filename << "\".\n";
      }
   }
	
   /**
	 * Step 3: For each swath, place points and corresponding
	 * metadata into tile files */
   ...
   metadataTypes[31109] = TdpDataType( 31108, "RGBA_COLOR_8", TypeChar8 );
   ...
   cout << "file_info_list.size() = " << file_info_list.size() << endl;
   for (unsigned int file_index=0; file_index<file_info_list.size(); 
        file_index++)
   {
      FileInfo&	file_info = file_info_list[file_index];
      ...
      if ( input_file.file_is_open() == false ) {
         cerr << "Could not open file \"" << file_name << "\". Skipping...\n";
      ...
      cout << "--- Sorting file \"" << file_name << "\". (" << file_index+1
           << " of " << file_info_list.size() << ")\n";
      ...
      if ( tdp_contents[ TdpKeyXYZ_POINT_DATA ] ) {
         count = input_file.klv_length( TdpKeyXYZ_POINT_DATA, 0 ) / 
            sizeof(float) / 3;
         points->resize( count );
			
         input_file.klv_read( 
            TdpKeyXYZ_POINT_DATA, 0, 
            (float *)points->getDataPointer(), count * 3 * sizeof(float) );
      }

      if ( tdp_contents[ TdpKeyALIRT_XYZP ] ) {
         count = input_file.klv_length( TdpKeyALIRT_XYZP, 0 ) / 
            sizeof(float) / 4;
         points->resize( count );
			
         // save XYZP stream into a float buffer
         float *	buffer = new float[count * 4];
         input_file.klv_read( TdpKeyALIRT_XYZP, 0, buffer, 
         ...
         for( unsigned long i = 0; i < count; i++ ) {
            (*points)[i].set( buffer[4*i + 0], buffer[4*i + 1], 
                              buffer[4*i + 2] );
      ...
      // load in metadata
      for( mdtIter = metadataTypes.begin(); mdtIter != metadataTypes.end(); 
           mdtIter++ )
      {
         ...
         {
				// expected size?
            unsigned int local_count = input_file.klv_length( key, 0 ) / 
               mdtIter->second.elementSizeBytes();
				
            if ( local_count != count ) {
               cerr << "In file \"" << file_name << "\", the metadata type " 
                    << mdtIter->second.name
            ...
            switch( mdtIter->second.type ) {
               case TypeChar8:
               {	
                  osg::UByteArray* typeArray = new osg::UByteArray;
                  typeArray->resize( count );
                  input_file.klv_read( 
                     key, 0, (char8_t *)typeArray->getDataPointer(), 
                     count * sizeof(char8_t) );
                  data = typeArray;
                  break;
               }
               case TypeUint16:
               {	
                  ...
                  input_file.klv_read( 
                     key, 0, (uint16_t *)
                     typeArray->getDataPointer(), count * sizeof(uint16_t) );
                  data = typeArray;
                  break;
               }
               case TypeUint32:
               {	
                  ...
                  typeArray->resize( count );
                  input_file.klv_read( 
                     key, 0, (uint32_t *)typeArray->getDataPointer(), 
               ...
               {	
                  ...
                  typeArray->resize( count );
                  input_file.klv_read( 
                     key, 0, (float *)typeArray->getDataPointer(), 
                     count * sizeof(float) );
                  data = typeArray;
                  break;
               }
...
//                 << endl;
               ...
               for( mdIter = metadata.begin(); mdIter != metadata.end(); mdIter++ ) {
                  ...
                  if ( ! tile->metadata[key].valid() ) {
                     switch( metadataTypes[key].type ) {
                        case TypeChar8: tile->metadata[key] = new osg::UByteArray; break;
                  ...
                  switch( metadataTypes[key].type ) {
                     case TypeChar8:
                     {	
                        const osg::UByteArray * source = static_cast<const osg::UByteArray *>( mdIter->second.get() );
                        osg::UByteArray * destination = static_cast<osg::UByteArray *>( tile->metadata[key].get() );
      ...
      for( tileIter = tiles.begin(); tileIter != tiles.end(); tileIter++ )
      {
         ...
         if ( tile->vertices.valid() ) {
            ...
            cout << "tile->filename = " << tile->filename << endl;
            ...
            if ( ! output_file.file_is_open() ) {
               cerr << "Could not open output file \"" << tile->filename << "\". Buffer will not be written to disk.\n";
            } else {
                  ...
                  switch( metadataTypes[key].type ) {
                     ...
                     {	
                        output_file.klv_write( key, klv_metadata_index, 
                                               (char8_t *)tile->metadata[key]->getDataPointer(), flush_count * sizeof(char8_t) );
                        break;
                     }
                     case TypeUint16:
                     {	
               ...
               cout << "Wrote " << flush_count << " points to \"" << tile->filename << "\".\n";
            }
   ...
   for( tileIter = tiles.begin(); tileIter != tiles.end(); tileIter++ )
   {
      ...
      if ( tile->has_been_flushed_to_disk ) {			
         ...
         if ( ! buffer_file.file_is_open() ) {
            cerr << "Could not open tile buffer file \"" << tile->filename << "\". Could not condense its contents.\n";
            continue;
         }
         ...
         if ( ! output_file.file_is_open() ) {
            cerr << "Could not create output file \"" << filename << "\".\n";
            continue;
         } else {
				// write UTM zone
            if ( unified.isDefined() ) 
            {
               string utmZone = unified.getUTMString();
               output_file.klv_create( TdpKeyUTMZone, 3 * sizeof(char) );
               output_file.klv_write( 
                  TdpKeyUTMZone, 0, 
                  (char *)utmZone.c_str(), 3 * sizeof(char) );
            }
 ...
 // write the unified theta and offset into the file
            if ( fabs(unifiedAngle) > 0.001 ) 
            {
               ...
               output_file.klv_write( TdpKeyALIRT_THETA, 0, &theta, 
                                      sizeof(real64_t) );
            }

            if ( unifiedOffset.length() > 0.001 ) {
               output_file.klv_create( 
                  TdpKeyUTMOffset, 3 * sizeof(real64_t) );
               output_file.klv_write( 
                  TdpKeyUTMOffset, 0, 
                  unifiedOffset.ptr(), 3 * sizeof(real64_t) );
            }
				
				// write the min and max keys
            float	min[3];
            float	max[3];
            min[0] = tile->extent.xMin();	
            min[1] = tile->extent.yMin();	
            min[2] = tile->extent.zMin();
            max[0] = tile->extent.xMax();	
            max[1] = tile->extent.yMax();	
            max[2] = tile->extent.zMax();
				
            output_file.klv_create( 
               TdpKeyALIRT_MIN, 3 * sizeof(real32_t) );
            output_file.klv_write( 
               TdpKeyALIRT_MIN, 0, min, 3 * sizeof(real32_t) );

            output_file.klv_create( 
               TdpKeyALIRT_MAX, 3 * sizeof(real32_t) );
            output_file.klv_write( 
               TdpKeyALIRT_MAX, 0, max, 3 * sizeof(real32_t) );
