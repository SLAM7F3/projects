// ========================================================================
// Program DBSAURON is a variant of SAURON which pulls aircraft and
// camera metadata from the FLIR database rather than from flat text
// files.
// ========================================================================
// Last updated on 10/27/11; 10/28/11; 10/31/11
// ========================================================================
...
//   double mission_start_time,mission_stop_time;
//   databasefunc:: retrieve_mission_start_stop_times_from_database(
//      FLIR_database_ptr,campaign_ID,mission_ID,
//      mission_start_time,mission_stop_time);
//   cout.precision(14);
//   cout << "mission_start_time = " << mission_start_time << endl;
//   cout << "mission_stop_time = " << mission_stop_time << endl;
...
// Instantiate PolyLines decorations group:
   ...
   vector<string> footprint_filenames=filefunc::files_in_subdir(
      footprints_subdir);
   for (unsigned int f=0; f<footprint_filenames.size(); f++)
   {
      vector<threevector> vertices;
      filefunc::ReadInfile(footprint_filenames[f]);
      for (unsigned int f=0; f<filefunc::text_line.size(); f++)
      {
...
// ========================================================================
// Retrieve aircraft and camera metadata from FLIR database for
// current campaign and mission:

   vector<int> frame_ID;
   vector<string> frame_prefix;
   vector<double> curr_time;
   vector<double> aircraft_lon,aircraft_lat,aircraft_alt;
   vector<double> aircraft_roll,aircraft_pitch,aircraft_yaw;
   databasefunc::retrieve_aircraft_metadata_from_database(
      FLIR_database_ptr,campaign_ID,mission_ID,
      frame_ID,curr_time,frame_prefix,
      aircraft_lon,aircraft_lat,aircraft_alt,
      aircraft_roll,aircraft_pitch,aircraft_yaw);

   vector<double> horiz_FOV,vert_FOV,camera_az,camera_el,camera_roll;
   databasefunc::retrieve_camera_metadata_from_database(
      FLIR_database_ptr,campaign_ID,mission_ID,
      horiz_FOV,vert_FOV,camera_az,camera_el,camera_roll);

   vector<rpy> curr_RPY;
   vector<twovector> curr_FOV;
   vector<threevector> curr_posn,curr_AER;
   
   int n_frames=frame_ID.size();
   for (int i=0; i<n_frames; i++)
   {
      geopoint curr_geopt(aircraft_lon[i],aircraft_lat[i],aircraft_alt[i]);
      curr_posn.push_back(curr_geopt.get_UTM_posn());
      curr_RPY.push_back(
         rpy(aircraft_roll[i],aircraft_pitch[i],aircraft_yaw[i]));
      curr_FOV.push_back(twovector(horiz_FOV[i],vert_FOV[i]));
      curr_AER.push_back(
         threevector(camera_az[i],camera_el[i],camera_roll[i]));
   }
...
// Recover flight number and date string from missions table within
// FLIR database:

   string start_date;
   databasefunc::retrieve_mission_metadata_from_database(
      FLIR_database_ptr,campaign_ID,mission_ID,flight_number,start_date);
   vector<string> time_substrings=stringfunc::decompose_string_into_substrings(
      start_date," -");
   string date_string=time_substrings[0]+time_substrings[1]+time_substrings[2];
   cout << "date_string = " << date_string << endl;

   string frames_subdir=
      "/home/cho/programs/c++/svn/projects/src/mains/thunderstorm/video_data/";
   frames_subdir += "mission_"+stringfunc::integer_to_string(mission_ID,3)+"/";
   cout << "frames_subdir = " << frames_subdir << endl;
...
//   Movie* movie_ptr=
      MoviesGroup_ptr->generate_new_Movie(texture_rectangle_ptr);
