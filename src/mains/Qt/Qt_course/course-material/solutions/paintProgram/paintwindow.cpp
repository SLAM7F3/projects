...
void PaintWindow::slotConfigure()
{
    if ( !_configDialog) {
        ...
        connect( _configDialog, SIGNAL( setPen( int, Qt::PenStyle ) ),
                 _scribbleArea, SLOT( slotSetPen( int, Qt::PenStyle ) ) );
        connect( _configDialog, SIGNAL( setBrush( Qt::BrushStyle ) ),
                 _scribbleArea, SLOT( slotSetBrush( Qt::BrushStyle ) ) );
