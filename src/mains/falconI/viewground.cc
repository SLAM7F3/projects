...
//			viewground filtered_points.osga


// ========================================================================
// Last updated on 12/1/11; 12/5/11; 12/6/11
// ========================================================================
...
// Use an ArgumentParser object to manage the program arguments:
   ...
   string ground_surface_filename=filtered_file_dir+"ground_surface.dat";
   filefunc::ReadInfile(ground_surface_filename);
   vector<threevector> ground_XYZ;

   double ground_thickness=0;	// meters
   banner="Enter ground surface thickness in meters:";
   outputfunc::write_big_banner(banner);
   cin >> ground_thickness;
   for (unsigned int i=0; i<filefunc::text_line.size(); i++)
   {
      vector<double> column_values=stringfunc::string_to_numbers(
         filefunc::text_line[i]);
      ground_XYZ.push_back(
         threevector(column_values[0],column_values[1],column_values[2]+
            ground_thickness));
   }
   cout << "ground_XYZ.size() = " << ground_XYZ.size() << endl;
...
// Generate points decoration group:

   osgGeometry::PointsGroup* PointsGroup_ptr=
      decorations.add_Points(ndims,passes_group.get_pass_ptr(cloudpass_ID),
      NULL);
   PointsGroup_ptr->set_crosshairs_size(1);

   bool draw_text_flag=false;
   for (unsigned int i=0; i<ground_XYZ.size(); i++)
   {
      osgGeometry::Point* Point_ptr=PointsGroup_ptr->generate_new_Point(
         ground_XYZ[i],draw_text_flag);
   } // loop over index i labeling ground points
