// ==========================================================================
// VERTICAL_STRING_IMAGES is a specialized variant of
// GENERATE_STRING_IMAGES which exports synthetic text images and
// accompanying masks that are vertically oriented.  Multiple
// instances of VERTICAL_STRING_IMAGES can be run in parallel on a
// single CPU machine.

//			 ./vertical_string_images

// ==========================================================================
// Last updated on 4/17/16; 4/18/16; 4/22/16
// ==========================================================================
...
// confirmed as "reasonable":
   ...
   string vertical_fonts_subdir="./fonts/reasonable_fonts/";
   ...
   vector<string> font_paths=filefunc::
      files_in_subdir_matching_specified_suffixes(
         allowed_suffixes,vertical_fonts_subdir,search_all_children_dirs_flag);
...
// STEP 1. Select text label and font:
      ...
      int min_n_chars = 2;
      int max_n_chars = 20;
      curr_imagetext.set_vertical_text_flag(true);

      textfunc::select_text_phrase(
         cleaned_strings, min_n_words, max_n_words, min_n_chars, max_n_chars,
         curr_imagetext);
...
// chip:
      ...
      {
         text_label_width = 3 * character_width;
      }
...
//      cout << "estimated_n_textlines = " << estimated_n_textlines << endl;
      ...
      string curr_phrase = curr_imagetext.get_phrase();
      if(curr_imagetext.get_vertical_text_flag())
      {
         curr_phrase = textfunc::vertical_text_label(curr_phrase);
      }
...
// On 4/17/16, we discovered the hard and painful way that our current
// method of adding carriage returns after each char in a phrase does
// NOT yield vertically oriented text for a nontrivial number of
// fonts.  So we explicitly check the final synthetic chip's height vs
// its width.  If the latter exceeds the former, delete the synthetic
// chip and start all over...

      unsigned int syn_chip_width, syn_chip_height;
      imagefunc::get_image_width_height(
         synthetic_chip_filename, syn_chip_width, syn_chip_height);

      if( (syn_chip_width > syn_chip_height) ||
          (int(syn_chip_width) != curr_imagetext.get_image_width()) ||
          (int(syn_chip_height) != curr_imagetext.get_image_height()) )
      {
//         cout << "syn_chip_width = " << syn_chip_height
//              << " syn_chip_height = " << syn_chip_height << endl;
         string unix_cmd="/bin/rm "+synthetic_chip_filename;
...
// Step 10:  Rotate image chip and masks:
      ...
      double max_abs_az = 30;
      double max_abs_el = 15;
      double max_abs_roll = 45;
