...
int main (int argc, char* argv[])
{
   ...
   string input_filename="saturations.dat";
   filefunc::ReadInfile(input_filename);

   const double min_x = 0.10325;
...
*/
   ...
   double lambda = 4.6399;
   double min_saturation = 0;
   double max_saturation = 1;
   double curr_saturation;

   int n_iters = 20000;
   vector<double> generated_saturations;
   for(int iter = 0; iter < n_iters; iter++)
   {
      curr_saturation = min_saturation + nrfunc::expdev(lambda);
      generated_saturations.push_back(curr_saturation);
   }      

   prob_distribution prob_saturations(generated_saturations, 200);
//   prob_distribution prob_saturations(generated_saturations, 500);
   prob_saturations.set_xmin(min_saturation);
   prob_saturations.set_xmax(max_saturation);
   prob_saturations.set_xtic(0.1);
   prob_saturations.set_xsubtic(0.05);
   prob_saturations.set_color(colorfunc::red);
