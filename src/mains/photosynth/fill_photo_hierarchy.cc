// ========================================================================
// Program FILL_PHOTO_HIERARCHY reads in the graph edge list
...
// Cluster Algorithm or K-means algorithm.  FILL_PHOTO_HIERARCHY
// writes out a set of SQL script files which populate the
// graphs, nodes, links and graph_annotations tables in
// the imagery database.  Each level within the graph pyramid contains
// node, edge and connected component information which can be parsed
// and visualized by Michael Yee's graph viewer.

//    fill_photo_hierarchy --region_filename ./bundler/MIT2317/packages/peter_inputs.pkg --GIS_layer ./packages/imagery_metadata.pkg

// ========================================================================
// Last updated on 6/20/13; 7/22/13; 10/30/13
// ========================================================================
...
//   cout << "bundler_IO_subdir = " << bundler_IO_subdir << endl;
   ...
   int hierarchy_ID=-1;
   cout << "Enter ID for graph hierarchy corresponding to specified campaign and mission:" << endl;
   cin >> hierarchy_ID;
...
// components per graph level:
   ...
   int minimal_edge_weights_threshold=0;
...
// component in graph gx,gy coordinates:
      ...
      if (cc_retrieval_flag && cc_label.size() > 0)
      {
...
//      sql_filenames.push_back(links_sql_filename);
      ...
      for (int s=0; s<sql_filenames.size(); s++)
      {
...
*/
   {
      string banner="Manually run SQL scripts insert_all_graphs, insert_all_nodes, insert_all_links and insert_cc_annotations!";
      outputfunc::write_banner(banner);
   }
