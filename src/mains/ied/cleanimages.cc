...
//   const double delta_y=0.3;	// meters
   ...
   cityimage.compute_trivial_xy_data_bbox(cityimage.z2Darray_ptr);
...
//                                cityimage.z2Darray_ptr);
   ladarfunc::remove_isolated_outliers(
      cityimage.get_data_bbox_ptr(),cityimage.z2Darray_ptr);

// Plot height distribution for image with original ground surface:

   ladarfunc::compute_z_distribution(
      cityimage.imagedir,cityimage.z2Darray_ptr);
   double mean_z=57.59;
   double max_z=mean_z+2;
   double min_z=mean_z-2;
   imagefunc::threshold_intensities_above_cutoff(
      cityimage.z2Darray_ptr,max_z,ladarimage::null_value);
   imagefunc::threshold_intensities_below_cutoff(
      cityimage.z2Darray_ptr,min_z,ladarimage::null_value);
...
//      cityimage.z2Darray_ptr,filled_zimage_filename);
...
//      filled_zimage_filename,100,cityimage.z2Darray_ptr,50);
...
// Create binary mask of partially cleaned height image:
   ...
   twoDarray* mask_twoDarray_ptr=new twoDarray(cityimage.z2Darray_ptr);
   binaryimagefunc::binary_threshold(
      0.5*ladarimage::null_value,cityimage.z2Darray_ptr,
      mask_twoDarray_ptr,ladarimage::null_value);

   int n_iters=5;
   int n_size=5;
   graphicsfunc::turtle_erode_binary_region(
      n_iters,n_size,mask_twoDarray_ptr,ladarimage::null_value);

   string mask_filename=cityimage.get_imagedir()+"mask.xyzp";
   ladarfunc::write_xyzp_data(
      cityimage.z2Darray_ptr,mask_twoDarray_ptr,mask_filename);
...
// destructively processsed:
   ...
   twoDarray* zground_silhouetted_twoDarray_ptr=
      new twoDarray(cityimage.z2Darray_ptr);
   cityimage.z2Darray_ptr->copy(zground_silhouetted_twoDarray_ptr);
...
// within median filled height image:
   ...
   twoDarray* xderiv_twoDarray_ptr=new twoDarray(cityimage.z2Darray_ptr);
   twoDarray* yderiv_twoDarray_ptr=new twoDarray(cityimage.z2Darray_ptr);
   ...
   twoDarray* zlevel_twoDarray_ptr=
      cityimage.interpolate_and_flatten_ground_surface(
         cityimage.z2Darray_ptr,zground_silhouetted_twoDarray_ptr,
         cityimage.get_p2Darray_ptr(),15);
   delete zground_silhouetted_twoDarray_ptr;
   recursivefunc::binary_null(
      0.5*ladarimage::null_value,zlevel_twoDarray_ptr,
      cityimage.z2Darray_ptr,ladarimage::null_value);
   ...
   ladarfunc::write_xyzp_data(
      ...
      zlevel_filename);
   draw3Dfunc::append_fake_z_points_in_twoDarray_middle(
      cityimage.z2Darray_ptr,zlevel_filename);
...
//      zhilo_twoDarray_ptr,cityimage.get_p2Darray_ptr(),zhilo_filename);
   ...
   ladarfunc::write_xyzp_data(
      ...
      flat_filenamestr);
   draw3Dfunc::append_fake_z_points_in_twoDarray_middle(
      cityimage.z2Darray_ptr,flat_filenamestr);

// Set all entries within refined ground mask image corresponding to
// relatively high objects equal to ladarimage::null_value:

   twoDarray* groundmask_twoDarray_ptr=new twoDarray(cityimage.z2Darray_ptr);
   cityimage.z2Darray_ptr->copy(groundmask_twoDarray_ptr);
   recursivefunc::binary_null(
      1.0,groundmask_twoDarray_ptr,zhilo_twoDarray_ptr,ladarimage::null_value,
      true);
   ...
   ladarfunc::write_xyzp_data(
      ...
      groundmask_filename);
   draw3Dfunc::append_fake_z_points_in_twoDarray_middle(
      cityimage.z2Darray_ptr,groundmask_filename);
...
// silhouetted ground mask:
   ...
   twoDarray* zlevel2_twoDarray_ptr=
      cityimage.interpolate_and_flatten_ground_surface(
         cityimage.z2Darray_ptr,groundmask_twoDarray_ptr,
         cityimage.get_p2Darray_ptr(),3);
   delete groundmask_twoDarray_ptr;
   recursivefunc::binary_null(
      0.5*ladarimage::null_value,zlevel2_twoDarray_ptr,
      cityimage.z2Darray_ptr,ladarimage::null_value);
   ...
   draw3Dfunc::append_fake_z_points_in_twoDarray_middle(
      cityimage.z2Darray_ptr,zlevel2_filename);

   zlevel2_twoDarray_ptr->copy(cityimage.z2Darray_ptr);
...
// pixels:
   ...
   twoDarray* ptwoDarray_lo_ptr=
      featurefunc::distinguish_road_and_grass_pixels(
         nlo_iters,cityimage.imagedir,p_tall_sentinel_value,
         zcompletely_flattened_twoDarray_ptr,cityimage.get_p2Darray_ptr());
...
// interested in long, connected swaths of roadside:
   ...
   string lo_features_filenamestr=cityimage.imagedir+"lo_features.xyzp";
   ladarfunc::write_xyzp_data(
      cityimage.z2Darray_ptr,lo_features_twoDarray_ptr,
...
// locations where the height function rapidly fluctuates:
   ...
   twoDarray* norm_fluc_twoDarray_ptr=
      cityimage.compute_z_fluctuations(cityimage.z2Darray_ptr);
...
// distinguish buildings from trees:
   ...
   twoDarray* ptwoDarray_hi_ptr=
      featurefunc::distinguish_tree_from_bldg_pixels(
         nhi_iters,cityimage.imagedir,p_low_sentinel_value,
         zcompletely_flattened_twoDarray_ptr,cityimage.get_p2Darray_ptr(),
         norm_fluc_twoDarray_ptr);
   delete norm_fluc_twoDarray_ptr;

   string p_trees_filenamestr=cityimage.imagedir+"p_trees.xyzp";
   ladarfunc::write_xyzp_data(
   ...
   twoDarray* zconnected_components_twoDarray_ptr=
      featurefunc::locate_building_clusters(
         cityimage.imagedir,ptwoDarray_hi_ptr,
...
//         cityimage.get_connected_heights_hashtable_ptr());
   ...
   twoDarray* zbuilding_twoDarray_ptr=new twoDarray(cityimage.z2Darray_ptr);
   ...
   string zbldg_filenamestr=cityimage.imagedir+"z_building.xyzp";
   ladarfunc::write_xyzp_data(
      cityimage.z2Darray_ptr,zbuilding_twoDarray_ptr,zbldg_filenamestr,false);
...
//   string fuse_filenamestr=cityimage.imagedir+"fuse_merge_10.xyzp";
//   ladarfunc::write_xyzp_data(
...
//      v_hi,v_lo,zmax,zmin,zcompletely_flattened_twoDarray_ptr,
//      pmerge_twoDarray_ptr);
//   fuse_filenamestr=cityimage.imagedir+"fuse_merge_01.xyzp";
//   ladarfunc::write_xyzp_data(
...
// roadsides in separate colors:
   ...
   string features_filenamestr=cityimage.imagedir+"features.xyzp";
   ladarfunc::write_xyzp_data(
      cityimage.z2Darray_ptr,features_twoDarray_ptr,features_filenamestr);
...
// rooftop information from the feature map:
   ...
   twoDarray* pbinary_twoDarray_ptr=new twoDarray(cityimage.z2Darray_ptr);
...
//   string binary_bldg_filename=cityimage.imagedir+"binary_bldg.xyzp";   
//   ladarfunc::write_xyzp_data(
//      cityimage.z2Darray_ptr,pbinary_new_twoDarray_ptr,binary_bldg_filename,
//      false);
...
// containing linkedlists of connected rooftop pixels:
   ...
   string connected_filename=cityimage.imagedir+"connected.xyzp";   
   ladarfunc::write_xyzp_data(
      cityimage.z2Darray_ptr,pconnected_twoDarray_ptr,connected_filename,
      false);
...
// Identify buildings with multi-tiered rooftops:
   ...
   vector<pair<int,vector<double> > >* tierred_bldg_ptr=
      featurefunc::detect_tiered_roofs(
         connected_components_hashtable_ptr,cityimage.z2Darray_ptr);
...
// individual rooftops:
   ...
   twoDarray* p_refined_roof_twoDarray_ptr=
      featurefunc::refine_building_extraction(
         cityimage.imagedir,tierred_bldg_ptr,
         connected_components_hashtable_ptr,
         cityimage.z2Darray_ptr,pconnected_twoDarray_ptr);
...
// asphalt regions:
   ...
   string refined_features_filename=cityimage.imagedir+
...
// corresponding to sites within the buildings network:
   ...
   cityimage.generate_buildings_network(
      min_footprint_area,cityimage.z2Darray_ptr,
...
// than as rooftop pixel clumps:
   ...
   xderiv_twoDarray_ptr=new twoDarray(cityimage.z2Darray_ptr);
   yderiv_twoDarray_ptr=new twoDarray(cityimage.z2Darray_ptr);
   imagefunc::compute_x_y_deriv_fields(
      spatial_resolution,cityimage.z2Darray_ptr,xderiv_twoDarray_ptr,
...
// in buildings network onto features map:
   ...
   featurefunc::recolor_feature_pixels(
      ...
      refined_features_twoDarray_ptr);
   cityimage.draw_building_site_pixels(refined_features_twoDarray_ptr);
   string final_features_filename=cityimage.imagedir+"final_features.xyzp";
   ladarfunc::write_xyzp_data(
      cityimage.z2Darray_ptr,refined_features_twoDarray_ptr,
      final_features_filename);
   draw3Dfunc::append_fake_xyzp_points_in_twoDarray_middle(
      cityimage.z2Darray_ptr,final_features_filename);
