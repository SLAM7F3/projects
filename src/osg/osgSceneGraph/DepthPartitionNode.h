...
class CURRENT_CLASS : public osg::Group
{
  public:
	...
	/** Specify whether the color buffer should be cleared before
	    the first CameraNode draws its scene. */
	...
	/** Catch child management functions so the CameraNodes can be
	    informed of added or removed children. */
	virtual bool addChild(osg::Node *child);
	virtual bool insertChild(unsigned int index, osg::Node *child);
	virtual bool removeChildren(
           unsigned int pos, unsigned int numRemove = 1);
