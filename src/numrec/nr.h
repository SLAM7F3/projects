...
// ==================================================================== 
// Last updated on 8/25/05
// ==================================================================== 

// CAUTION: This is the ANSI C (only) version of the Numerical Recipes
// utility file numrec/nr.h.  Do not confuse this file with the same-named
// file numrec/nr.h that is supplied in the 'misc' subdirectory.  *That* file
// is the one from the book, and contains both ANSI and traditional
// K&R versions, along with #ifdef macros to select the correct
// version.  *This* file contains only ANSI C.
...
namespace numrec
{
   ...
   void banmul(double **a, unsigned long n, int m1, int m2, 
               double x[], double b[]);
...
// On 9/8/99, PLC added the pointer *hnew to the list of arguments for
//   subroutine odeint:

   void odeint(double ystart[], int nvar, double x1, double x2,
               double eps, double h1, double hmin, double *hnew,int *nok, 
               int *nbad,
...
// ====================================================================
// The following methods are not (yet) included within the numrec
// namespace, for we do (or at least should) not use these methods.
// As of 2003, we are attempting to minimize our dependence upon
...
double erff_nr(double x);

// Note: We renamed NR routine erff on 11/16/98 as erff_nr in order to 
// avoid a conflict with the erf routine in /usr/include/mathcalls.h
// It should *NOT* be necessary to make this change, since erf is not the
// same thing as erff.  But for now, we have made this change.
...
void quadvl(double x, double y, double *fa, double *fb, 
            double *fc, double *fd);
