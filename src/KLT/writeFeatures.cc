...
#include "KLT/base.h"
#include "KLT/error.h"
#include "KLT/pnmio.h"		/* ppmWriteFileRGB() */
#include "KLT/klt.h"
  ...
  char *filename)
{
  ...
  if (redimg == NULL || grnimg == NULL || bluimg == NULL)
    KLTError( (char*) "(KLTWriteFeaturesToPPM)  Out of memory\n");

  /* Copy grey image to component images */
  if (sizeof(KLT_PixelType) != 1)
     KLTWarning( (char*) "(KLTWriteFeaturesToPPM)  KLT_PixelType is not uchar");
  ...
  for (i = 0 ; i < featurelist->nFeatures ; i++)
    if (featurelist->feature[i]->val >= 0)  {
      ...
      for (yy = y - 1 ; yy <= y + 1 ; yy++)
	    ...
	    if (featurelist->feature[i]->val > 0)
	      *(grnimg + offset) = 255; // yellow if NEW
	    else
	      *(grnimg + offset) = 0; // red if old
  ...
  char *type)	/* Output: either 'f' or 'd', based on input format */
{
  ...
  if (fp == NULL)
    KLTError( (char*) "(KLTWriteFeatures) "
             "Can't open file '%s' for writing\n", fname);

  /* Parse format */
  if (fmt[0] != '%')
    KLTError( (char*) "(KLTWriteFeatures) Bad Format: %s\n", fmt);
  i = 0;  while (fmt[i] != '\0') i++;  *type = fmt[i-1];
  if (*type != 'f' && *type != 'd')
    KLTError( (char*) "(KLTWriteFeatures) Format must end in 'f' or 'd'.");
  ...
  char *fname) 	/* Input: filename */
{
  FILE *fp;
  if (fname == NULL) 
    KLTError( (char*) "(KLTWriteFeatures) Can't write binary data to stderr");
  fp = fopen(fname, "wb");
  if (fp == NULL)
    KLTError( (char*) "(KLTWriteFeatures) "
    ...
    if (str[i] == '%')  {
      if (isdigit(str[i+1]))  {
        ...
        while (!_isCharInString(str[i], "diouxefgn"))  {
          i++;
          if (i > maxi)
            KLTError( (char*) "(_findStringWidth) Can't determine length "
                     "of string '%s'", str);
        }
        i++;
      } else if (str[i+1] == 'c')  {
...
#define LINELENGTH 100
  ...
  while (strcmp(line, warning_line) != 0)  {
    ...
    if (feof(fp))
      KLTError( (char*) "(_readFeatures) File is corrupted -- Couldn't find line:\n"
               "\t%s\n", warning_line);
  }
  ...
  if (strcmp(line, "KLT Feature List\n") == 0) id = FEATURE_LIST;
    ...
    KLTError( (char*) "(_readFeatures) File is corrupted -- (Not 'KLT Feature List', "
  ...
  if (id == FEATURE_LIST)  {
    if (strcmp(line, "nFeatures") != 0)
      KLTError( (char*) "(_readFeatures) File is corrupted -- "
  ...
  if (id == FEATURE_TABLE)  {
    fscanf(fp, "%s", line);
    if (strcmp(line, ",") != 0)
      KLTError( (char*) "(_readFeatures) File '%s' is corrupted -- "
  ...
  char *fname)
{
  ...
  fp = fopen(fname, "rb");
  if (fp == NULL)  KLTError( (char*) "(KLTReadFeatureList) Can't open file '%s' "
                            "for reading", fname);
  if (KLT_verbose >= 1) 
    fprintf(stderr,  "(KLT) Reading feature list from '%s'\n", fname);
  id = _readHeader(fp, NULL, &nFeatures, &binary);
  if (id != FEATURE_LIST) 
    KLTError( (char*) "(KLTReadFeatureList) File '%s' does not contain "
             "a FeatureList", fname);
  ...
  if (fl_in == NULL)  {
    ...
    if (fl->nFeatures != nFeatures)
      KLTError( (char*) "(KLTReadFeatureList) The feature list passed "
  ...
  if (!binary) {  /* text file */
    for (i = 0 ; i < fl->nFeatures ; i++)  {
      fscanf(fp, "%d |", &indx);
      if (indx != i) KLTError( (char*) "(KLTReadFeatureList) Bad index at i = %d"
                              "-- %d", i, indx);
  ...
  char *fname)
{
  ...
  fp = fopen(fname, "rb");
  if (fp == NULL)  KLTError( (char*) "(KLTReadFeatureHistory) Can't open file '%s' "
                            "for reading", fname);
  if (KLT_verbose >= 1) fprintf(stderr,  "(KLT) Reading feature history from '%s'\n", fname);
  id = _readHeader(fp, &nFrames, NULL, &binary);
  if (id != FEATURE_HISTORY) KLTError( (char*) "(KLTReadFeatureHistory) File '%s' does not contain "
  ...
  if (fh_in == NULL)  {
    ...
    if (fh->nFrames != nFrames)
      KLTError( (char*) "(KLTReadFeatureHistory) The feature history passed "
  ...
  if (!binary) {  /* text file */
    for (i = 0 ; i < fh->nFrames ; i++)  {
      fscanf(fp, "%d |", &indx);
      if (indx != i) 
        KLTError( (char*) "(KLTReadFeatureHistory) Bad index at i = %d"
                 "-- %d", i, indx);
  ...
  char *fname)
{
  ...
  fp = fopen(fname, "rb");
  if (fp == NULL)  KLTError( (char*) "(KLTReadFeatureTable) Can't open file '%s' "
                            "for reading", fname);
  if (KLT_verbose >= 1) fprintf(stderr,  "(KLT) Reading feature table from '%s'\n", fname);
  id = _readHeader(fp, &nFrames, &nFeatures, &binary);
  if (id != FEATURE_TABLE) KLTError( (char*) "(KLTReadFeatureTable) File '%s' does not contain "
  ...
  if (ft_in == NULL)  {
    ...
    if (ft->nFrames != nFrames || ft->nFeatures != nFeatures)
      KLTError( (char*) "(KLTReadFeatureTable) The feature table passed "
  ...
  if (!binary) {  /* text file */
    for (j = 0 ; j < ft->nFeatures ; j++)  {
      fscanf(fp, "%d |", &indx);
      if (indx != j) 
        KLTError( (char*) "(KLTReadFeatureTable) Bad index at j = %d"
                 "-- %d", j, indx);
      for (i = 0 ; i < ft->nFrames ; i++)
