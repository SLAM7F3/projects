...
#include "KLT/base.h"
#include "KLT/error.h"
#include "KLT/convolve.h"	/* for computing pyramid */
#include "KLT/klt.h"
#include "KLT/klt_util.h"	/* _KLT_FloatImage */
#include "KLT/pyramid.h"	/* _KLT_Pyramid */
   ...
   int height)
{
   ...
   if (fw == NULL)  KLTError( (char*) "(_allocateFloatWindow) Out of memory.");
   return fw;
}
 ...
 *
 * Tracks a feature point from the image of first occurrence to the
 * actual image.
 *
 * RETURNS
   ...
   KLT_FeatureList featurelist)
{
   ...
   if (tc->sequentialMode && tc->pyramid_last != NULL)  {
      ...
      if (pyramid1->ncols[0] != ncols || pyramid1->nrows[0] != nrows)
         KLTError( (char*) "(KLTTrackFeatures) Size of incoming image (%d by %d) "
