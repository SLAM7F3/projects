...
modify, and distribute this software. STANFORD MAKES NO
...
static const int affine_max_residue = 10.0;
...
static void** _createArray2D(int ncols, int nrows, int nbytes)
{
   ...
   tt = (char **) malloc(nrows * sizeof(void *) +
                         ncols * nrows * nbytes);
   if (tt == NULL)
      KLTError( (char*) "(createArray2D) Out of memory");
...
void KLTChangeTCPyramid(KLT_TrackingContext tc,int search_range)
{
   ...
   if (tc->window_width % 2 != 1) {
      ...
      KLTWarning( (char*) "(KLTChangeTCPyramid) Window width must be odd.  "
                 "Changing to %d.\n", tc->window_width);
   }
   if (tc->window_height % 2 != 1) {
      tc->window_height = tc->window_height+1;
      KLTWarning( (char*) "(KLTChangeTCPyramid) Window height must be odd.  "
                 "Changing to %d.\n", tc->window_height);
   }
   if (tc->window_width < 3) {
      tc->window_width = 3;
      KLTWarning( (char*) "(KLTChangeTCPyramid) Window width must be at least three.  \n"
                 "Changing to %d.\n", tc->window_width);
   }
   if (tc->window_height < 3) {
      tc->window_height = 3;
      KLTWarning( (char*) "(KLTChangeTCPyramid) Window height must be at least three.  \n"
                 "Changing to %d.\n", tc->window_height);
   }
...
void KLTUpdateTCBorder(KLT_TrackingContext tc)
{
   ...
   if (tc->window_width % 2 != 1) {
      tc->window_width = tc->window_width+1;
      KLTWarning( (char*) "(KLTUpdateTCBorder) Window width must be odd.  "
                 "Changing to %d.\n", tc->window_width);
   }
   if (tc->window_height % 2 != 1) {
      tc->window_height = tc->window_height+1;
      KLTWarning( (char*) "(KLTUpdateTCBorder) Window height must be odd.  "
                 "Changing to %d.\n", tc->window_height);
   }
   if (tc->window_width < 3) {
      tc->window_width = 3;
      KLTWarning( (char*) "(KLTUpdateTCBorder) Window width must be at least three.  \n"
                 "Changing to %d.\n", tc->window_width);
   }
   if (tc->window_height < 3) {
      tc->window_height = 3;
      KLTWarning( (char*) "(KLTUpdateTCBorder) Window height must be at least three.  \n"
                 "Changing to %d.\n", tc->window_height);
   }
