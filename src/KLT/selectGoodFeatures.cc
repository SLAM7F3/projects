...
{
  while (n > 1) {
    /* searching for pivot works somewhat better
       (especially since we are sorting non-unique data) */
    /* SWAP3(pointlist, 0, n/2); */
    {
      register int p0,p1,ind,endInd;
      ind=2;
      p0=pointlist[ind];
      ind+=3;
      p1=pointlist[ind];
      endInd=3*n-1;
      while ((p1 >= p0) && (ind < endInd)) {
	p0=p1;
	ind+=3;
	p1=pointlist[ind];
      }
      if (p1 < p0) {
	SWAP3(pointlist, 0, (ind-2)/3);
      } else {
	return;
      }
    }
    ...
    {
      register unsigned int i, j, ln, rn;
...
{
  return (gxx + gyy - fsqrt((gxx - gyy)*(gxx - gyy) + 4*gxy*gxy))/2.0;
}
  ...
  selectionMode mode)
{
  ...
  if (tc->window_width % 2 != 1) {
    ...
    KLTWarning( (char*) "Tracking context's window width must be odd.  "
               "Changing to %d.\n", tc->window_width);
  }
  if (tc->window_height % 2 != 1) {
    tc->window_height = tc->window_height+1;
    KLTWarning( (char*) "Tracking context's window height must be odd.  "
               "Changing to %d.\n", tc->window_height);
  }
  if (tc->window_width < 3) {
    tc->window_width = 3;
    KLTWarning( (char*) "Tracking context's window width must be at least three.  \n"
               "Changing to %d.\n", tc->window_width);
  }
  if (tc->window_height < 3) {
    tc->window_height = 3;
    KLTWarning( (char*) "Tracking context's window height must be at least three.  \n"
               "Changing to %d.\n", tc->window_height);
  }
  ...
  if (tc->writeInternalImages)  {
     _KLTWriteFloatImageToPGM(floatimg, (char *) "kltimg_sgfrlf.pgm");
     _KLTWriteFloatImageToPGM(gradx, (char *) "kltimg_sgfrlf_gx.pgm");
     _KLTWriteFloatImageToPGM(grady, (char *) "kltimg_sgfrlf_gy.pgm");
  }
     ...
     of the two eigenvalues of the Z matrix */
  {
    ...
    for (unsigned i = 0 ; i < sizeof(int) ; i++)  limit *= 256;
    limit = limit/2 - 1;
    ...
    for (y = bordery ; y < nrows - bordery ; y += tc->nSkippedPixels + 1)
      for (x = borderx ; x < ncols - borderx ; x += tc->nSkippedPixels + 1)  {
        ...
        *ptr++ = y;
        ...
        if (val > limit)  {
          KLTWarning( (char*) "(_KLTSelectGoodFeatures) minimum eigenvalue %f is "
  ...
  /* Check tc->mindist */
  if (tc->mindist < 0)  {
    KLTWarning( (char*) "(_KLTSelectGoodFeatures) Tracking context field tc->mindist "
  ...
  KLT_FeatureList fl)
{
  ...
  if (KLT_verbose >= 1 && nLostFeatures > 0)
  {
     cout << endl;
     cout << "**********************************************************"
          << endl;
     cout << "Attempting to replace " << nLostFeatures << " features" << endl;
     cout << "**********************************************************"
          << endl;
...
//    fflush(stderr);
  ...
  if (KLT_verbose >= 1)  
  {
     int n_features_replaced=nLostFeatures - fl->nFeatures 
        + KLTCountRemainingFeatures(fl);
     if (n_features_replaced > 0)
     {
        fprintf(stderr,  "\n\t%d features replaced.\n",n_features_replaced);
     }
