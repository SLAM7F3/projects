  ...
  int nlevels)
{
  ...
  if (subsampling != 2 && subsampling != 4 && 
    ...
    KLTError( (char*) "(_KLTCreatePyramid)  Pyramid's subsampling must "
  ...
  if (pyramid == NULL)
    KLTError( (char*) "(_KLTCreatePyramid)  Out of memory");
  ...
  float sigma_fact)
{
  ...
  if (subsampling != 2 && subsampling != 4 && 
      subsampling != 8 && subsampling != 16 && subsampling != 32)
    KLTError( (char*) "(_KLTComputePyramid)  Pyramid's subsampling must "
             "be either 2, 4, 8, 16, or 32");
