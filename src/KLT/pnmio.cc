  ...
  int *maxval)
{
  ...
  if (line[0] != 'P')
     KLTError((char *) "(pnmReadHeader) Magic number does not begin with 'P', "
             "but with a '%c'", line[0]);
  ...
  *nrows = atoi(line);
  if (*ncols < 0 || *nrows < 0 || *ncols > 10000 || *nrows > 10000)
     KLTError((char *) "(pnmReadHeader) The dimensions %d x %d are unacceptable",
             *ncols, *nrows);
  ...
  *maxval = atoi(line);
  ...
  if (*maxval != 255)
     KLTWarning((char *) "(pnmReadHeader) Maxval is not 255, but %d", *maxval);
}
  ...
  int *maxval)
{
  ...
  if (*magic != 5)
     KLTError((char *) "(pgmReadHeader) Magic number is not 'P5', but 'P%d'", *magic);
}
  ...
  int *maxval)
{
  pnmReadHeader(fp, magic, ncols, nrows, maxval);
  if (*magic != 6)
     KLTError((char *) "(ppmReadHeader) Magic number is not 'P6', but 'P%d'", *magic);
}
  ...
  int *maxval)
{
  ...
  if ( (fp = fopen(fname, "rb")) == NULL)
     KLTError((char *) "(pgmReadHeaderFile) Can't open file named '%s' for reading\n", fname);
  ...
  int *maxval)
{
  ...
  /* Open file */
  if ( (fp = fopen(fname, "rb")) == NULL)
     KLTError((char *) "(ppmReadHeaderFile) Can't open file named '%s' for reading\n", fname);
  ...
  int *ncols, int *nrows)
{
  ...
  if (img == NULL)  {
    ...
    if (ptr == NULL)  
       KLTError((char *) "(pgmRead) Memory not allocated");
  }
  else
    ptr = img;
  ...
  int *ncols, int *nrows)
{
  ...
  /* Open file */
  if ( (fp = fopen(fname, "rb")) == NULL)
    KLTError( (char*) "(pgmReadFile) Can't open file named '%s' for reading\n", fname);
  ...
  int nrows)
{
  ...
  if ( (fp = fopen(fname, "wb")) == NULL)
    KLTError( (char*) "(pgmWriteFile) Can't open file named '%s' for writing\n", fname);
  ...
  int nrows)
{
  ...
  /* Open file */
  if ( (fp = fopen(fname, "wb")) == NULL)
    KLTError( (char*) "(ppmWriteFileRGB) Can't open file named '%s' for writing\n", fname);
