// -------------------------------------------------------------------
// Sksfunc namespace methods that enable communication with and
// manipulation of old SKS database
// -------------------------------------------------------------------
// Last updated on 10/19/06; 10/27/06; 6/21/07
// -------------------------------------------------------------------
...
//   static PGconn* g_connection = NULL;

   static const int kEntityIdBufSize = 32;

   static const string insert_comment_preamble = "INSERT INTO comment (identifier, dt_created, source_url, data_url, metadata_url, mimetype, comment_typ, name, text, labeltextx, labeltexty, labeltype, linecolor, movieclip, id, poi_id, textbackgroundcolor, textcolor, url, username, x, y, xscale, yscale, zoom) VALUES (";

   static const string insert_relation_preamble = "INSERT INTO e_relation (src_id, src_type, tgt_id, tgt_type, typ) VALUES (";

   static const string sql_quote = "'";
   static const string sql_comma = ",";

   static const int comment_type_poi = 0;
   static const int comment_type_label = 1;
   static const int comment_type_note = 2;
...
//  function.
   ...
   static void initEntityIdSequence (
   ...
   static bool Setup (void)
      {
         ...
         char connect_flags[256];
         memset(connect_flags, 0, sizeof(connect_flags));

         sprintf(connect_flags, "host = %s dbname = %s user = %s\n", 
                 g_settings.hostName,g_settings.dbName,g_settings.userName);
    
         g_connection = PQconnectdb(connect_flags);

         if (!g_connection)
            return false;
    
         // Check the status of the connection from libpq.
         ConnStatusType status = PQstatus(g_connection);
         ...
         initEntityIdSequence(
            g_connection, lastEntityId, sizeof(lastEntityId));
...
// -------------------------------------------------------------------
   static void addRelation (PGconn* db_connection, 
...
// -------------------------------------------------------------------
   static int getNextCommentId (PGconn* db_connection, 
                                const char* imageId, 
                                int poi_id,
                                int comment_type)
      {
...
// -------------------------------------------------------------------
static bool imageHasWholeImagePOI (
...
// -------------------------------------------------------------------
   static void addWholeImagePoiComment (
...
// -------------------------------------------------------------------
   static void addPoiComment (PGconn* db_connection, 
                              const char* imageId, 
                              const char* poiName)
      {
         // Get the next id
...
// -------------------------------------------------------------------
   ...
   static void addNoteComment (PGconn* db_connection, 
                               const char* imageId, 
                               const char* noteName, 
                               const char* noteText,
                               int poiId)
...
// -------------------------------------------------------------------
   static string addLabelComment (PGconn* db_connection, 
                                  const char* imageId, 
                                  int poi_id, 
                                  string labelName,
                                  double pixelX,
                                  double pixelY)
      {
         ...
         sql_buffer += sql_quote + labelName + sql_quote 
            + string(",");  // --name 
         sql_buffer += "'',";      //  --text
...
// red dots which are roughly 10 meters in size on the single
...
//         cout << sql_buffer << endl;
         ...
         return string(nextEntityId);
      }

// -------------------------------------------------------------------
   static string addZoomifyComment (database* old_isds_database_ptr,
                                    ...
                                    double pixelY)
      {
         ...
         if (!imageHasWholeImagePOI(
            old_isds_database_ptr->get_db_connection_ptr(),
            imageEntityId, 
            existing_poi_id))
         {
            ...
            // be ID zero. 
            addWholeImagePoiComment(
               old_isds_database_ptr->get_db_connection_ptr(), imageEntityId);
         }
...
//                        existing_poi_id);
         ...
         return addLabelComment(
            old_isds_database_ptr->get_db_connection_ptr(), 
            imageEntityId, existing_poi_id, 
...
//  Place the result in the vector of image ids and geometries. 
   ...
   static bool QueryImagesContainingLatLon (
      ...
      double lat, double lon, vector<ImageGeometry>& ig)
      {
         ...
         PGresult* res = PQexec(
            old_isds_database_ptr->get_db_connection_ptr(),szQueryBuffer);


    
         // There shouldn't be a NULL result unless something really
         // dire happens.  libpq will only return NULL If we're out of
         // memory.  Nevertheless, we should indicate to the program
         ...
         {
            temp_ig.imageId = PQgetvalue(res, i, 0);
            temp_ig.wktGeometry = PQgetvalue(res, i, 1);
            temp_ig.pixelWidth = atoi(PQgetvalue(res, i, 2));
            temp_ig.pixelHeight = atoi(PQgetvalue(res, i, 3));
...
// worry about selecting an actual pixel location
   ...
   static void LatLongToPixelLocation (float x, float y, 
...
// method...
   ...
   static void LatLongToPixelLocation (
...
*/
   ...
   static bool LatLonToPixelCoordsForImage (double lat, double lon, 
                                            double& pixelX, double& pixelY)
      {
...
// -------------------------------------------------------------------
   static bool AddCommentAtPixelCoordinates (string& imageId, 
                                             string comment, 
                                             int pixelX, 
                                             int pixelY)
      {
...
// -------------------------------------------------------------------
   static void GetGeometryCentroid (
...
//  Return true if that imageId is found, otherwise return false. 
   ...
   static bool TestResultsForImageId (const vector<ImageGeometry>& ig, 
...
// -------------------------------------------------------------------
   static bool TestQueryForImageCentroids (void)
      {
         ...
         while (iter != end)
         {
            ...
            double cx = 0;
            double cy = 0;
            string wktGeometry = imag_geom.wktGeometry;
            GetGeometryCentroid(imag_geom.wktGeometry, cx, cy);
            ...
            if (!TestResultsForImageId(test_results, imag_geom.imageId))
            {
               printf("Warning - %s - did not contain it's centroid\n", 
                      imag_geom.imageId.c_str());
            }
         }
...
// -------------------------------------------------------------------
   string AddSksAnnotation (
      database* old_isds_database_ptr,
      double lat, double lon, string labelName)
      {
         string SKS_label_ID="";
...
*/
         ...
         QueryImagesContainingLatLon(old_isds_database_ptr,lat,lon,results);
...
//                   pixelY);
            ...
            SKS_label_ID=addZoomifyComment (
               old_isds_database_ptr,
               ig.imageId.c_str(),
               labelName,pixelX,pixelY);
         }
...
// -------------------------------------------------------------------
   void DeleteSksAnnotation(
      database* old_isds_database_ptr,string SKS_label_ID)
      {
         string delete_comment_sql_command=
            "DELETE FROM comment WHERE identifier = '"+
            SKS_label_ID+"';";
         string delete_relation_sql_command=
            "DELETE FROM e_relation WHERE tgt_id = '"+
            SKS_label_ID+"';";
         
//         cout << "delete comment command = " 
//              << delete_comment_sql_command << endl;
//         cout << "delete reln command = "
//              << delete_relation_sql_command << endl;

         PQexec(
            old_isds_database_ptr->get_db_connection_ptr(),
            delete_comment_sql_command.c_str());

         PQexec(old_isds_database_ptr->get_db_connection_ptr(),
                delete_relation_sql_command.c_str());
      }

// -------------------------------------------------------------------
   void UpdateSksAnnotationName(
      database* old_isds_database_ptr,      
      string SKS_label_ID,string newName)
      {
         string update_comment_sql_command=
            "UPDATE comment SET name = '"+newName
            +"' WHERE identifier = '"+SKS_label_ID+"';";

//         cout << "update_comment_sql_command = "
//              << update_comment_sql_command << endl;
         
         PQexec(old_isds_database_ptr->get_db_connection_ptr(),
                update_comment_sql_command.c_str());
      }
