...
static void free_ntuple_list(ntuple_list in)
{
  if( in == NULL || in->values == NULL )
    error( (char*) "free_ntuple_list: invalid n-tuple input.");
  free( (void *) in->values );
  free( (void *) in );
}
...
static ntuple_list new_ntuple_list(unsigned int dim)
{
  ...
  /* check parameters */
  if( dim == 0 ) error( (char*) "new_ntuple_list: 'dim' must be positive.");

  /* get memory for list structure */
  n_tuple = (ntuple_list) malloc( sizeof(struct ntuple_list_s) );
  if( n_tuple == NULL ) error( (char*) "not enough memory.");
  ...
  if( n_tuple->values == NULL ) error( (char*) "not enough memory.");
...
static void enlarge_ntuple_list(ntuple_list n_tuple)
{
  ...
  if( n_tuple == NULL || n_tuple->values == NULL || n_tuple->max_size == 0 )
    error( (char*) "enlarge_ntuple_list: invalid n-tuple.");
...
{
  /* check parameters */
   if( out == NULL ) error( (char*) "add_7tuple: invalid n-tuple input.");
   if( out->dim != 7 ) error( (char*) "add_7tuple: the n-tuple must be a 7-tuple.");

  /* if needed, alloc more tuples to 'out' */
  if( out->size == out->max_size ) enlarge_ntuple_list(out);
  if( out->values == NULL ) error( (char*) "add_7tuple: invalid n-tuple input.");
...
static void free_image_char(image_char i)
{
  if( i == NULL || i->data == NULL )
    error( (char*) "free_image_char: invalid input image.");
  free( (void *) i->data );
  free( (void *) i );
}
...
static image_char new_image_char(unsigned int xsize, unsigned int ysize)
{
  ...
  /* check parameters */
  if( xsize == 0 || ysize == 0 ) error( (char*) "new_image_char: invalid image size.");

  /* get memory */
  image = (image_char) malloc( sizeof(struct image_char_s) );
  if( image == NULL ) error( (char*) "not enough memory.");
  image->data = (unsigned char *) calloc( (size_t) (xsize*ysize),
                                          sizeof(unsigned char) );
  if( image->data == NULL ) error( (char*) "not enough memory.");
                                      ...
                                      unsigned char fill_value )
{
  ...
  if( image == NULL || image->data == NULL )
    error( (char*) "new_image_char_ini: invalid image.");
...
static image_int new_image_int(unsigned int xsize, unsigned int ysize)
{
  ...
  /* check parameters */
  if( xsize == 0 || ysize == 0 ) error( (char*) "new_image_int: invalid image size.");
...
{
  if( i == NULL || i->data == NULL )
    error( (char*) "free_image_double: invalid input image.");
  free( (void *) i->data );
  free( (void *) i );
}
...
static image_double new_image_double(unsigned int xsize, unsigned int ysize)
{
  ...
  /* check parameters */
  if( xsize == 0 || ysize == 0 ) error( (char*) "new_image_double: invalid image size.");
                                          ...
                                          unsigned int ysize, double * data )
{
  ...
  /* check parameters */
  if( xsize == 0 || ysize == 0 )
    error( (char*) "new_image_double_ptr: invalid image size.");
  if( data == NULL ) error( (char*) "new_image_double_ptr: NULL data pointer.");
...
static void gaussian_kernel(ntuple_list kernel, double sigma, double mean)
{
  ...
  if( kernel == NULL || kernel->values == NULL )
    error( (char*) "gaussian_kernel: invalid n-tuple 'kernel'.");
                                      ...
                                      double sigma_scale )
{
  ...
  if( in == NULL || in->data == NULL || in->xsize == 0 || in->ysize == 0 )
    error( (char*) "gaussian_sampler: invalid image.");
  if( scale <= 0.0 ) error( (char*) "gaussian_sampler: 'scale' must be positive.");
  if( sigma_scale <= 0.0 )
    error( (char*) "gaussian_sampler: 'sigma_scale' must be positive.");

  /* compute new image size and get memory for images */
  if( in->xsize * scale > (double) UINT_MAX ||
      in->ysize * scale > (double) UINT_MAX )
    error( (char*) "gaussian_sampler: the output image size exceeds the handled size.");
                              ...
                              image_double * modgrad, unsigned int n_bins )
{
  ...
  /* check parameters */
  if( in == NULL || in->data == NULL || in->xsize == 0 || in->ysize == 0 )
    error( (char*) "ll_angle: invalid image.");
  if( threshold < 0.0 ) error( (char*) "ll_angle: 'threshold' must be positive.");
  if( list_p == NULL ) error( (char*) "ll_angle: NULL pointer 'list_p'.");
  if( mem_p == NULL ) error( (char*) "ll_angle: NULL pointer 'mem_p'.");
  if( modgrad == NULL ) error( (char*) "ll_angle: NULL pointer 'modgrad'.");
  if( n_bins == 0 ) error( (char*) "ll_angle: 'n_bins' must be positive.");
                      ...
                      double prec )
{
  ...
  if( angles == NULL || angles->data == NULL )
    error( (char*) "isaligned: invalid image 'angles'.");
  if( x < 0 || y < 0 || x >= (int) angles->xsize || y >= (int) angles->ysize )
    error( (char*) "isaligned: (x,y) out of the image.");
  if( prec < 0.0 ) error( (char*) "isaligned: 'prec' must be positive.");
...
static double nfa(int n, int k, double p, double logNT)
{
  ...
  if( n<0 || k<0 || k>n || p<=0.0 || p>=1.0 )
    error( (char*) "nfa: wrong n, k or p values.");
...
{
  /* check parameters */
  if( in == NULL || out == NULL ) error( (char*) "rect_copy: invalid 'in' or 'out'.");
...
static double inter_low(double x, double x1, double y1, double x2, double y2)
{
  /* check parameters */
  if( x1 > x2 || x < x1 || x > x2 )
    error( (char*) "inter_low: unsuitable input, 'x1>x2' or 'x<x1' or 'x>x2'.");
...
static double inter_hi(double x, double x1, double y1, double x2, double y2)
{
  /* check parameters */
  if( x1 > x2 || x < x1 || x > x2 )
    error( (char*) "inter_hi: unsuitable input, 'x1>x2' or 'x<x1' or 'x>x2'.");
...
{
  if( iter == NULL ) error( (char*) "ri_del: NULL iterator.");
  free( (void *) iter );
}
...
{
  /* check input */
  if( i == NULL ) error( (char*) "ri_end: NULL iterator.");
...
{
  /* check input */
  if( i == NULL ) error( (char*) "ri_inc: NULL iterator.");
...
static rect_iter * ri_ini(struct rect * r)
{
  ...
  /* check parameters */
  if( r == NULL ) error( (char*) "ri_ini: invalid rectangle.");

  /* get memory */
  i = (rect_iter *) malloc(sizeof(rect_iter));
  if( i == NULL ) error( (char*) "ri_ini: Not enough memory.");
...
static double rect_nfa(struct rect * rec, image_double angles, double logNT)
{
  ...
  /* check parameters */
  if( rec == NULL ) error( (char*) "rect_nfa: invalid rectangle.");
  if( angles == NULL ) error( (char*) "rect_nfa: invalid 'angles'.");
                         ...
                         image_double modgrad, double reg_angle, double prec )
{
  ...
  /* check parameters */
  if( reg == NULL ) error( (char*) "get_theta: invalid region.");
  if( reg_size <= 1 ) error( (char*) "get_theta: region size <= 1.");
  if( modgrad == NULL || modgrad->data == NULL )
    error( (char*) "get_theta: invalid 'modgrad'.");
  if( prec < 0.0 ) error( (char*) "get_theta: 'prec' must be positive.");
  ...
  if( double_equal(Ixx,0.0) && double_equal(Iyy,0.0) && double_equal(Ixy,0.0) )
    error( (char*) "get_theta: null inertia matrix.");
                         ...
                         double prec, double p, struct rect * rec )
{
  ...
  /* check parameters */
  if( reg == NULL ) error( (char*) "region2rect: invalid region.");
  if( reg_size <= 1 ) error( (char*) "region2rect: region size <= 1.");
  if( modgrad == NULL || modgrad->data == NULL )
    error( (char*) "region2rect: invalid image 'modgrad'.");
  if( rec == NULL ) error( (char*) "region2rect: invalid 'rec'.");
  ...
  if( sum <= 0.0 ) error( (char*) "region2rect: weights sum equal to zero.");
  x /= sum;
  y /= sum;
                         ...
                         double prec )
{
  ...
  /* check parameters */
  if( x < 0 || y < 0 || x >= (int) angles->xsize || y >= (int) angles->ysize )
    error( (char*) "region_grow: (x,y) out of the image.");
  if( angles == NULL || angles->data == NULL )
    error( (char*) "region_grow: invalid image 'angles'.");
  if( reg == NULL ) error( (char*) "region_grow: invalid 'reg'.");
  if( reg_size == NULL ) error( (char*) "region_grow: invalid pointer 'reg_size'.");
  if( reg_angle == NULL ) error( (char*) "region_grow: invalid pointer 'reg_angle'.");
  if( used == NULL || used->data == NULL )
    error( (char*) "region_grow: invalid image 'used'.");
                                 ...
                                 double density_th )
{
  ...
  /* check parameters */
  if( reg == NULL ) error( (char*) "reduce_region_radius: invalid pointer 'reg'.");
  if( reg_size == NULL )
    error( (char*) "reduce_region_radius: invalid pointer 'reg_size'.");
  if( prec < 0.0 ) error( (char*) "reduce_region_radius: 'prec' must be positive.");
  if( rec == NULL ) error( (char*) "reduce_region_radius: invalid pointer 'rec'.");
  if( used == NULL || used->data == NULL )
    error( (char*) "reduce_region_radius: invalid image 'used'.");
  if( angles == NULL || angles->data == NULL )
    error( (char*) "reduce_region_radius: invalid image 'angles'.");
                   ...
                   image_char used, image_double angles, double density_th )
{
  ...
  /* check parameters */
  if( reg == NULL ) error( (char*) "refine: invalid pointer 'reg'.");
  if( reg_size == NULL ) error( (char*) "refine: invalid pointer 'reg_size'.");
  if( prec < 0.0 ) error( (char*) "refine: 'prec' must be positive.");
  if( rec == NULL ) error( (char*) "refine: invalid pointer 'rec'.");
  if( used == NULL || used->data == NULL )
    error( (char*) "refine: invalid image 'used'.");
  if( angles == NULL || angles->data == NULL )
    error( (char*) "refine: invalid image 'angles'.");
                               ...
                               int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  /* check parameters */
  if( img == NULL || X <= 0 || Y <= 0 ) error( (char*) "invalid image input.");
  if( scale <= 0.0 ) error( (char*) "'scale' value must be positive.");
  if( sigma_scale <= 0.0 ) error( (char*) "'sigma_scale' value must be positive.");
  if( quant < 0.0 ) error( (char*) "'quant' value must be positive.");
  if( ang_th <= 0.0 || ang_th >= 180.0 )
    error( (char*) "'ang_th' value must be in the range (0,180).");
  if( density_th < 0.0 || density_th > 1.0 )
    error( (char*) "'density_th' value must be in the range [0,1].");
  if( n_bins <= 0 ) error( (char*) "'n_bins' value must be positive.");
  ...
  */
  ...
  if( reg == NULL ) error( (char*) "not enough memory!");
  ...
  if( reg_img != NULL && reg_x != NULL && reg_y != NULL )
    {
      if( region == NULL ) error( (char*) "'region' should be a valid image.");
      *reg_img = region->data;
      if( region->xsize > (unsigned int) INT_MAX ||
          region->xsize > (unsigned int) INT_MAX )
        error( (char*) "region image to big to fit in INT sizes.");
  ...
  if( out->size > (unsigned int) INT_MAX )
    error( (char*) "too many detections to fit in an INT.");
  *n_out = (int) (out->size);
